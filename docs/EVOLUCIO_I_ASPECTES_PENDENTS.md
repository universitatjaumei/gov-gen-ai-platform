...
11. **Desenvolupar fora, registrar dins: quins dels sis candidats de `docs/GOVERNANCA_PER_API.md` s'incorporen al pla, i en quin orde?** **Recomanació:** els tres de cost baix i que ja existixen com a funció (contracte de cites, vigència d'un document, auditoria estàtica com a servici) com un bloc curt després de REG; els assistents externs registrats amb escenaris com a segon pas, perquè és el que contesta «els xatbots els fem nosaltres» amb un «sí, i la plataforma els assaja i els registra igual que als seus»; el depòsit de manifests i els paràmetres de governança quan hi haja la primera aplicació externa real que els necessite. I una petició concreta a la UADTI: que diga si vol l'auditoria estàtica com a servici, perquè és la seua revisió posterior i la seua caixa d'eines, no les nostres.  
