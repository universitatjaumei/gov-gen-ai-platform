...
### Qüestió 7 · experimentar, governar, i que desenvolupament desenvolupe

## La plataforma no compet amb la UADTI pel desenvolupament: li dóna on registrar-lo

Les reticències de desenvolupament tenen, crec, un fons raonable que convé dir en veu alta: **accepten que es governe i que s'experimente, però entenen que desenvolupar és cosa seua**. Els xatbots els poden fer ells, i els informes amb IA també. No cal discutir-ho, perquè la plataforma no està pensada per a impedir-ho. Està pensada per a tres coses, i cap és «desenvolupar en lloc de desenvolupament»:

1. **Experimentar**: assajar un assistent sobre corpus real amb escenaris, veredicte humà, dataset dorat i el prompt exacte a la vista, abans que ningú el done per bo.
2. **Fer executable el marc de governança** (*compliance as code*): les obligacions del Reglament d'IA, del RGPD i de la Llei 40/2015 traduïdes a comprovacions que corren en cada petició i a registres que es generen per construcció.
3. **Governar el desenvolupament ciutadà** del nivell 2 de la Instrucció 02/2026 (Qüestió 4).

I la frase que tanca la discussió: **una vegada els paràmetres de governança d'un cas d'ús estan definits i provats a la plataforma, la UADTI pot desenvolupar-lo fora, amb les seues eines, sempre que l'aplicació registre per API a la plataforma el que el marc exigix registrar** i use per API les comprovacions que no té sentit reimplementar. El nivell 3 de la Instrucció pot viure dins o fora; el registre viu a un sol lloc. Hui ja es pot construir una interfície pròpia sobre el motor de xat per API amb token personal i heretar tota la governança sense programar res més.

L'inventari complet (mecanisme, garantia, norma, on viu, i si s'hi accedix des de fora hui, si està planificat en REG o FUN, o si és candidat) està a **`docs/GOVERNANCA_PER_API.md`**. El que hi ha de nou són **sis candidats** que són justament el que falta perquè la frase anterior siga literal (tres ja planificats com a bloc VAS, tres pendents de decidir): lectura dels paràmetres de governança d'un cas d'ús, depòsit del manifest d'una execució feta fora, assistents externs registrats als quals la plataforma passa els escenaris i registra el veredicte (la variant (b) de la decisió 8 vista des de la governança), el contracte de cites com a servici, l'estat de vigència d'un document del corpus, i **l'auditoria estàtica com a servici**, que és la revisió posterior del nivell 2 per a codi que no corre a la plataforma i el candidat que millor encaixa amb el paper que la Instrucció dóna a la UADTI. Tots van després del bloc REG, que construïx el que tots reutilitzen.
...
9. **Perfils i estratègies de tercers: repositori o plugins de desplegament?** Els pros i contres estan a la taula de la Qüestió 4 (continuació). **Recomanació revisada el 2 de setembre**, arreplegant la insistència de desenvolupament: el **descobriment per *entry points* s'afig ja**, i els perfils i estratègies del propi nucli entren pel mateix mecanisme, perquè això obliga el motor a no dependre de res que no passe pel registre; el que **no** es fa encara és congelar els protocols d'estratègia com a contracte públic, que han canviat dues vegades en un mes per mesura i es declaren explícitament inestables (0.x) fins que hi haja un tercer real. L'alta de codi de motor per la interfície en runtime queda descartada. **Ja és un bloc de treball planificat (PLG, tres passos)**, amb tres grups d'*entry points*: `govgenai.graph_profiles`, `govgenai.strategies` i `govgenai.retrieval_pipelines`. Per a les estratègies calia resoldre abans una cosa: una estratègia solta no la seleccionava la configuració, la componia un perfil de manera fixa. El bloc ho fa seleccionable **eix a eix**: els quatre protocols que compon el graf (recuperació, fusió, plantilla, llengua) tenen cada un el seu registre, el perfil declara una composició per defecte per nom, i l'organització o el xatbot poden sobreescriure una clau sense perfil nou ni codi. Una estratègia aportada per un paquet és seleccionable en instal·lar-se, i quina va córrer en cada resposta queda a la traça. El bucle agèntic no és un eix (el munta el mode de recuperació) i queda anotat com a candidat. També ha eixit que el frontend duia els noms de perfil i mode fixats al codi, que és justament el que un perfil instal·lat no podria travessar: el bloc ho corregix.
...
11. **Desenvolupar fora, registrar dins: quins dels candidats de `docs/GOVERNANCA_PER_API.md` s'incorporen al pla, i en quin orde?** **Ja decidit i planificat (2 de setembre):** els tres de cost baix que ja existixen com a funció (contracte de cites, vigència d'un document, auditoria estàtica com a servici amb la caixa d'eines publicada des del codi) són el **bloc VAS**, quatre passos després de REG. **Queda per decidir:** els assistents externs registrats amb escenaris com a pas següent, perquè és el que contesta «els xatbots els fem nosaltres» amb un «sí, i la plataforma els assaja i els registra igual que als seus»; el depòsit de manifests i els paràmetres de governança quan hi haja la primera aplicació externa real que els necessite. I una petició concreta a la UADTI: que diga si vol l'auditoria estàtica com a servici, perquè és la seua revisió posterior i la seua caixa d'eines, no les nostres.  
