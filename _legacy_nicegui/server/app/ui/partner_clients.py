...
def partner_clients_content(ctx: PartnerContext):
    ...
    async def load_clients():
        ...
        async with AsyncSession(server_engine) as session:
            ...
            if page_state.search_query:
                page_state.clients = await service.search_clients(
                ...
                if page_state.is_editing:
                    ...
                    if not page_state.form_permanent:
                        valid_date = datetime.strptime(
                            page_state.form_valid_until, "%Y-%m-%d"
                        )
                    ...
                    (
                        ...
                        license_obj,
                        plain_key,
                    ) = await service.create_client_with_license(
                        name=page_state.form_name,
                        nif=page_state.form_nif,
    ...
    with client_dialog_modal, ui.card().classes("w-full max-w-lg"):
        title_label = ui.label(t("partner.clients.title")).classes(
        ...
        id_input = (
            ui.input(t("partner.clients.client_id"))
        ...
        with ui.row().classes("w-full gap-2"):
            name_input = (
                ui.input(t("partner.clients.client_name"))
                .props("outlined")
                .classes("flex-1")
            )
            nif_input = (
                ui.input(t("partner.clients.client_nif"))
                .props("outlined")
                .classes("w-1/3")
            )
        ...
        with new_client_fields:
            ...
            quota_input = (
                ui.number(value=100000)
                .props("outlined suffix=Tokens")
                .classes("w-full")
            )

            ui.separator().classes("my-2")
            ui.label(t("partner.clients.expiration_date")).classes(
            ...
            with ui.row().classes("w-full items-center gap-4"):
                date_input = (
                    ui.input(
                        value=(datetime.now() + timedelta(days=365)).strftime(
                            "%Y-%m-%d"
                        )
                    )
        ...
        with ui.row().classes("w-full justify-end mt-4"):
            ui.button(t("common.cancel"), on_click=client_dialog_modal.close).props(
                "flat color=grey"
            )
            ui.button(t("common.save"), on_click=lambda: save_client_handler()).props(
                "color=primary"
            )
    ...
    def update_dialog_ui():
        ...
        title_label.text = (
            t("admin.common.edit")
            if page_state.is_editing
            else t("partner.clients.add_client")
        )
    ...
    with stats_dialog_modal, ui.card():
        ...
        ui.button(t("playwright_close"), on_click=stats_dialog_modal.close).classes(
            "mt-4"
        )
    ...
    def update_stats_ui():
        ...
        with stats_container:
            if page_state.stats_data:
                ui.label(
                    f"{t('partner.clients.quota_total')}: {page_state.stats_data.get('quota_tokens', 0):,}"
                )
                ui.label(
                    f"{t('partner.clients.consumed')}: {page_state.stats_data.get('consumed_tokens', 0):,}"
                )

                percent = page_state.stats_data.get("usage_percent", 0)
                ui.label(f"{t('partner.clients.usage')}: {percent}%").classes(
                    "font-bold"
                )
                ui.linear_progress(value=percent / 100).classes("w-full mt-2").props(
                    "color=purple"
                )
            else:
    ...
    with license_dialog_modal, ui.card():
        ui.label(t("partner.clients.new_key_title")).classes(
            "text-lg font-bold text-green-600"
        )
        ui.label(t("partner.clients.new_key_help")).classes("text-sm text-red-500 mb-2")
        license_code_ui = ui.code("").classes(
            "w-full p-2 bg-gray-100 rounded text-lg font-mono"
        )
        ui.button(
            t("partner.clients.understood"), on_click=license_dialog_modal.close
        ).classes("w-full mt-4")
    ...
    with details_dialog, ui.card().classes("w-full max-w-lg"):
        ...
        ui.button(t("playwright_close"), on_click=details_dialog.close).classes(
            "w-full mt-4"
        )
        ...
        async with AsyncSession(server_engine) as session:
            ...
            with details_container:
                ...
                if getattr(client, "nif", None):
                    with ui.row().classes("w-full justify-between"):
                        ui.label(t("partner.clients.client_nif")).classes(
                            "text-gray-600"
                        )
                ...
                if license:
                    with ui.row().classes("w-full justify-between"):
                        ...
                        ui.label(license.license_id).classes(
                            "font-mono font-bold text-blue-800"
                        )

                    with ui.row().classes("w-full justify-between"):
                        ui.label(t("partner.clients.valid_until")).classes(
                            "text-gray-600"
                        )
                        valid_str = (
                    ...
                    with ui.row().classes("w-full justify-between"):
                        ui.label(t("admin.partners.partner_status")).classes(
                            "text-gray-600"
                        )
                        ui.badge(license.status).props(
                            f"color={'green' if license.status == 'ACTIVE' else 'red'}"
                        )
                else:
    ...
    async def open_create_dialog():
        ...
        page_state.form_valid_until = (datetime.now() + timedelta(days=365)).strftime(
            "%Y-%m-%d"
        )
    ...
    async def regenerate_key_wrapper(client):
        async with AsyncSession(server_engine) as session:
            service = PartnerClientService(session, ctx.partner_id)
            try:
                new_key_plain, _ = await service.regenerate_license_key(
                    client.client_id
                )
    ...
    with ui.column().classes("w-full gap-4"):
        ...
        with ui.row().classes("w-full justify-between items-center"):
            ...
            ui.button(
                t("partner.clients.add_client"), icon="add", on_click=open_create_dialog
            ).props("color=primary")

        # Filter
        with ui.row().classes(
            "w-full items-center gap-2 bg-white p-2 rounded shadow-sm"
        ):
            ui.input(
                placeholder=t("partner.clients.search_placeholder"),
                on_change=lambda e: setattr(page_state, "search_query", e.value),
            ).bind_value(page_state, "search_query").classes("flex-1").props(
        ...
        def clients_table():
            ...
            if not page_state.clients:
                ui.label(t("partner.clients.no_clients")).classes(
                    "w-full text-center text-gray-500 py-8 italic"
                )
                return
            ...
            with ui.card().classes("w-full p-0 gap-0"):
                # Header Row
                with ui.row().classes(
                    "w-full bg-slate-100 p-3 font-bold text-gray-700 border-b"
                ):
                ...
                for client in page_state.clients:
                    with ui.row().classes(
                        "w-full p-3 border-b hover:bg-slate-50 items-center no-wrap"
                    ):
                        ...
                        ui.label(client.created_at.strftime("%Y-%m-%d")).classes(
                            "w-1/6 text-xs text-gray-500"
                        )

                        # Actions
                        with ui.row().classes(
                            "flex-1 justify-end gap-2 flex-nowrap items-center"
                        ):
                            ui.button(
                                icon="visibility",
                                on_click=lambda c=client: open_details_dialog(c),
                            ).props("flat dense round color=teal").tooltip(
                                t("dash_view_all")
                            )
                            ui.button(
                                icon="analytics",
                                on_click=lambda c=client: show_stats_wrapper(c),
                            ).props("flat dense round color=purple").tooltip(
                                t("partner.clients.stats_title")
                            )
                            ui.button(
                                icon="vpn_key",
                                on_click=lambda c=client: regenerate_key_wrapper(c),
                            ).props("flat dense round color=orange").tooltip(
                                t("partner.clients.generate_key")
                            )
                            ui.button(
                                icon="edit",
                                on_click=lambda c=client: open_edit_dialog(c),
                            ).props("flat dense round color=blue").tooltip(
                                t("common.edit")
                            )
                            if client.is_active:
                                ui.button(
                                    icon="block",
                                    on_click=lambda c=client: deactivate_client(c),
                                ).props("flat dense round color=red").tooltip(
                                    t("common.delete")
                                )
                            else:
                                ui.button(
                                    icon="restore",
                                    on_click=lambda c=client: reactivate_client(c),
                                ).props("flat dense round color=green").tooltip(
                                    "Reactivar Cliente"
                                )
