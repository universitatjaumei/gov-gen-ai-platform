...
def partner_library_content(ctx: PartnerContext):
    ...
    async def load_items():
        async with AsyncSession(server_engine) as session:
            ...
            all_visible = await service.get_visible_automations(
    ...
    async def delete_item():
        ...
        async with AsyncSession(server_engine) as session:
            ...
            success = await service.delete_automation(
                state.selected_item.id, ctx.partner_id
            )
    ...
    with ui.dialog() as new_dialog, ui.card().classes("w-96 p-6"):
        ...
        ui.label("Seleccione cómo desea crear el nuevo componente:").classes(
        ...
        with ui.column().classes("w-full gap-3"):
            with (
                ui.row()
                .classes(
                    "w-full items-center p-3 border rounded cursor-pointer hover:bg-slate-50"
                )
                .on("click", create_new_manual)
            ):
                ui.icon("edit", size="sm").classes("text-indigo-600")
                with ui.column().classes("gap-0"):
                    ui.label("Creación manual").classes("font-bold text-sm")
                    ui.label("Empezar desde un documento vacío").classes(
            ...
            ui.upload(
                on_upload=handle_import, auto_upload=True, label="Subir archivo"
            ).classes("w-full").props("accept=.json,.automatia")

    # 1. LIST (GALLERY)
    @ui.refreshable
    def list_grid():
        with ui.row().classes("w-full items-center justify-between mb-6"):
            ui.label("Mis automatismos (masters)").classes(
                "text-2xl font-bold text-slate-800"
            )
            ui.button("Nuevo", icon="add", on_click=new_dialog.open).props(
                "elevated color=primary"
            )

        if not state.items:
            with ui.card().classes(
                "w-full p-12 items-center justify-center bg-slate-50 border-dashed border-2"
            ):
                ui.icon("auto_awesome", size="3em").classes("text-slate-300 mb-2")
                ui.label("No tienes automatismos creados todavía.").classes(
        ...
        with ui.grid().classes(
            ...
            for item in state.items:
                ...
                with (
                    ui.card()
                    .classes(
                        f"w-full p-0 cursor-pointer hover:shadow-md transition-shadow transition-colors overflow-hidden border-l-4 border-{color}-500"
                    )
                    .on("click", lambda i=item: select_item(i))
                ):
                    with ui.row().classes("w-full p-4 items-start gap-3"):
                        ui.icon(icon, size="1.5em").classes(
                            f"text-{color}-600 bg-{color}-50 p-2 rounded"
                        )
                        with ui.column().classes("flex-1 gap-0"):
                            ui.label(item.name).classes(
                                "font-bold text-slate-800 line-clamp-1"
                            )
                            ui.label(type_val.lower().replace("_", " ")).classes(
                                "text-[10px] uppercase font-bold text-gray-400 tracking-wider"
                            )

                    with ui.row().classes(
                        "w-full px-4 py-2 bg-slate-50 justify-between items-center"
                    ):
    ...
    def detail_container():
        title = (
            "Nuevo automatismo"
            if not state.selected_item
            else f"Editando: {state.name}"
        )
        ui.label(title).classes("text-xl font-bold mb-6 text-slate-800")

        with ui.card().classes("w-full p-6"):
            with ui.column().classes("w-full gap-4"):
                ui.input("Nombre").bind_value(state, "name").classes("w-full").props(
                    "outlined dense"
                )

                ui.select(
                    options=[t.value for t in AutomationType]
                    + ["WORKFLOW", "PLAYBOOK"],
                    label="Tipo de automatismo",
                ).bind_value(state, "type").classes("w-full").props("outlined dense")

                ui.label("Código / contenido JSON").classes(
                    "text-sm font-bold text-slate-600 mt-2"
                )
                ui.codemirror(language="python").bind_value(state, "code").classes(
                    "h-96 border rounded w-full border-gray-200"
                )

                with ui.row().classes("w-full justify-between items-center mt-4"):
                    if state.selected_item:
                        ui.button("Eliminar", color="red", on_click=delete_item).props(
                            "flat dense"
                        )
                    else:
                        ui.label()  # Spacer
                    ...
                    with ui.row().classes("gap-2"):
                        ui.button(
                            "Cancelar",
                            on_click=lambda: (
                                setattr(state, "selected_item", None),
                                list_grid.refresh(),
                            ),
                        ).props("flat")
                        ui.button(
                            "Guardar cambios", icon="save", on_click=save_item
