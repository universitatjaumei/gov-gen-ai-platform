...
def get_status_label(license_obj: License) -> str:
    ...
    if license_obj.valid_until and datetime.utcnow() > license_obj.valid_until:
        return "EXPIRED"
    days_left = (license_obj.valid_until - datetime.utcnow()).days
    if days_left < 15:
...
def admin_licenses_content():
    ...
    with ui.card().classes("w-full p-4"):
        ui.label("Monitorización de Licencias y Consumo").classes(
        ...
        license_table = ui.table(
            columns=columns, rows=[], row_key="license_id"
        ).classes("w-full")
        ...
        with detail_dialog, ui.card().classes("w-full max-w-3xl"):
            ...
            def render_detail_content():
                ...
                ui.linear_progress(
                    value=row["usage_raw"], size="20px", show_value=False
                ...
                if consumption:
                    with ui.row().classes("w-full gap-4 mb-4"):
                        with ui.card().classes("flex-1 p-4 bg-blue-50"):
                            ...
                            ui.label(
                                f"{consumption.get('consumed_tokens', 0):,}"
                        ...
                        with ui.card().classes("flex-1 p-4 bg-purple-50"):
                            ...
                            ui.label(
                                f"{consumption.get('remaining_tokens', 0):,}"
                            ).classes("text-2xl font-bold text-purple-800")
        ...
        async def show_details(row):
            ...
            async with AsyncSession(server_engine) as session:
                ...
                detail_state["consumption"] = await service.get_license_summary(
                    row["license_id"]
                )

            render_detail_content.refresh()

        license_table.add_slot(
            "body-cell-status",
            r"""
            <q-td :props="props">
        ...
        async def refresh_licenses():
            async with AsyncSession(server_engine) as session:
                statement = (
                    select(License, ClientAccount, PartnerAccount)
        ...
        ui.button("Actualizar Tabla", icon="refresh", on_click=refresh_licenses).props(
            "flat dense"
        )
