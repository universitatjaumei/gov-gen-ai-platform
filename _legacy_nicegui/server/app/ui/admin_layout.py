...
def get_admin_sidebar_items() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        {
            "label": t("admin.menu_partners"),
            "icon": "business",
            "route": "/admin/partners",
        },
        {"label": t("admin.menu_clients"), "icon": "badge", "route": "/admin/clients"},
        {
            "label": t("admin.menu_licenses"),
            "icon": "verified_user",
            "route": "/admin/licenses",
        },
        {
            "label": t("admin.menu_security"),
            "icon": "security",
        ...
        {
            "label": t("admin.menu_ai_config"),
            "icon": "settings",
            "route": "/admin/ai-config",
        },
        {
            "label": t("admin.menu_prompts"),
            "icon": "psychology",
            "route": "/admin/prompts",
        },
    ]
    ...
    with ui.header().classes("bg-slate-900 text-white shadow-md items-center"):
        with ui.row().classes("w-full items-center justify-between px-4"):
            ...
            with ui.row().classes("items-center gap-4"):
                ...
                ui.button(icon="logout", on_click=lambda: ui.navigate.to("/")).props(
        ...
        with ui.column().classes("flex-1 h-full p-6 bg-white overflow-y-auto"):
            ...
            with ui.row().classes("mb-4 gap-2 text-sm text-gray-500"):
                bc = generate_breadcrumb(
                    app.storage.browser.get("current_path", "/admin")
                )
