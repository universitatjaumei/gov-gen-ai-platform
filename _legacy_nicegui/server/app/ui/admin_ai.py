...
def admin_ai_content():
    ...
    with ui.card().classes("w-full p-4 mb-6"):
        ...
        ui.label("Gestión de proveedores y modelos (Server-side)").classes(
            ...
            for role in get_roles(t):
                with ui.card().classes("p-4 hover:shadow-md transition-shadow"):
                    ...
                    async def on_provider_change(
                        e, r_key=role_key, m_ref=model_select_ref
                    ):
                        m_sel = m_ref["select"]
                        if not m_sel:
                            return
                    ...
                    async def load_current(
                        r_key=role["key"], p_sel=provider_select, m_sel=model_select
                    ):
                        async with AsyncSession(server_engine) as session:
                            ...
                            if conf:
                                ...
                                current_provider = "google"
                                current_model = DEFAULT_MODELS.get(r_key, {}).get(
                                    "google", ""
                                )
                            ...
                            if not models:
                                default = DEFAULT_MODELS.get(r_key, {}).get(
                                    current_provider, ""
                                )
                                models = [default] if default else []
                    ...
                    async def save_role_config(
                        r=role, p=provider_select, m=model_select
                    ):
                        async with AsyncSession(server_engine) as session:
                            ...
                            if not conf:
                                ...
                                session.add(conf)
                                print(
                                    f"[Admin AI] CREATED new config: role={r['key']}, provider={p.value}, model={m.value}"
                                )
                            else:
                                print(
                                    f"[Admin AI] UPDATING config: role={r['key']}, OLD provider={conf.provider}, NEW provider={p.value}"
                                )
                            ...
                            print(
                                f"[Admin AI] VERIFIED after commit: role={r['key']}, provider={verify.provider if verify else 'NOT FOUND'}"
                            )
                        ui.notify(
                            f"Configuración de {r['label']} guardada.", type="positive"
                        )
                    ...
                    async def test_api_connection(p=provider_select, m=model_select):
                        ...
                        try:
                            ...
                            if result.get("error"):
                                ...
                                ui.notify(
                                    f"Conexión exitosa con {provider}/{model}",
                                    type="positive",
                                )
                            else:
                                ui.notify(
                                    f"Sin respuesta de {provider}/{model}",
                                    type="warning",
                                )
                    ...
                    with ui.row().classes("w-full gap-2 mt-4"):
                        ui.button(icon="save", on_click=save_role_config).props(
                            "flat dense color=primary"
                        ).tooltip("Guardar configuración")
                        ui.button(icon="science", on_click=test_api_connection).props(
...
def render_scheduler_section(t):
    ...
    with ui.card().classes("w-full p-4 mt-6"):
        ...
        ui.label(t("admin.scheduler.section_desc")).classes(
            "text-sm text-gray-500 mb-4"
        )
            ...
            with ui.row().classes("w-full gap-6 items-start"):
                ...
                with ui.column().classes("flex-1 gap-4"):
                    ...
                    with ui.row().classes("items-center gap-4"):
                        ...
                        status_icon = (
                            "check_circle" if status.get("running") else "cancel"
                        )
                        ui.icon(status_icon).classes(
                            f"text-{status_color}-500 text-2xl"
                        )
                        ui.label(
                        ...
                        if status.get("next_run"):
                            ...
                            ui.label(
                                f"| {t('admin.scheduler.next_run')}: {next_run.strftime('%H:%M')}"
                    ...
                    with ui.row().classes("items-center gap-4"):
                        ...
                        ui.label(t("admin.scheduler.time_label")).classes(
                            "text-sm ml-4"
                        )
                        hour_select = (
                            ui.select(
                ...
                with ui.column().classes("gap-2"):
                    ...
                    async def run_manual_refresh():
                        ...
                        ui.notify(
                            t("admin.scheduler.manual_completed"), type="positive"
                        )
                        await load_scheduler_status()

                    ui.button(
                        t("common.save"), icon="save", on_click=save_scheduler_config
