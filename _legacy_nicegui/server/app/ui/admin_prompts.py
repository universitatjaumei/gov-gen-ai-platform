...
TIER_OPTIONS = [
    {
        "value": None,
        "key_label": "tier_default_label",
        "key_desc": "tier_default_desc",
        "badge": "⚪",
    },
...
def admin_prompts_content():
    ...
    PROMPT_METADATA = {
        ...
        "System: Precision Extraction (Fase 1)": {
            "task": "Extracción PDF",
            "phase": "Fase 1: Extracción de Precisión",
            "icon": "description",
            "color": "indigo",
            "sort_priority": 11,
        },
        "System: Refinement (Golden Record)": {
            "task": "Extracción PDF",
            "phase": "Fase 2: Refinamiento de Datos",
            "icon": "verified",
            "color": "green",
            "sort_priority": 12,
        },
        "System: Data Noise Filter": {
            "task": "Extracción PDF",
            "phase": "Filtrado de Ruido",
            "icon": "filter_alt",
            "color": "orange",
            "sort_priority": 13,
        },
        "System: Clarification (Extraction)": {
            "task": "Extracción PDF",
            "phase": "Clarificación",
        ...
        "System: RPA Refinement": {
            "task": "Navegación RPA",
            "phase": "Refinamiento de Selector",
            "icon": "ads_click",
            "color": "teal",
            "sort_priority": 21,
        },
        "System: RPA Vision": {
            "task": "Navegación RPA",
            "phase": "Visión Computacional",
            "icon": "visibility",
            "color": "cyan",
            "sort_priority": 22,
        },
        "System: Clarification (RPA)": {
            "task": "Navegación RPA",
            "phase": "Clarificación",
            "icon": "contact_support",
            "color": "cyan",
        ...
        "System: Script Factory (Phase 1)": {
            "task": "Transformación ETL",
            "phase": "Fase 1: Generación de Código",
            "icon": "code",
            "color": "pink",
            "sort_priority": 30,
        },
        "Auto-reparación y Refinamiento (Fase 3)": {
            "task": "Extracción PDF",
            "phase": "Fase 3: Auto-reparación de Script",
            "icon": "loop",
            "color": "purple",
            "sort_priority": 15,
        },
        ...
        "System: Audit Forensic (Phase 3)": {
            "task": "Extracción PDF",
            "phase": "Fase 3: Análisis de Calidad",
            "icon": "gavel",
            "color": "red",
            "sort_priority": 16,
        },
        "System: Script Factory (Phase 3)": {
            "task": "Extracción PDF",
            "phase": "Fase 3: Generación de Script",
            "icon": "factory",
            "color": "deep-purple",
            "sort_priority": 14,
        },
        "System: Clarification (ETL)": {
            "task": "Transformación ETL",
            "phase": "Clarificación",
            "icon": "contact_support",
            "color": "purple",
        ...
        "System: Custom Script Generator": {
            "task": "Personalizados",
            "phase": "Generación de Scripts",
            "icon": "auto_fix_high",
            "color": "pink",
            "sort_priority": 40,
        },
        "System: Clarification (Custom Script)": {
            "task": "Personalizados",
            "phase": "Clarificación",
            "icon": "contact_support",
            "color": "pink",
        ...
        "System: Flow Orchestrator": {
            "task": "Orquestador",
            "phase": "Orquestación de Flujos",
            "icon": "hub",
            "color": "red",
            "sort_priority": 50,
        },
    ...
    with ui.column().classes("w-full gap-4"):
        ...
        with ui.card().classes("w-full p-4 border border-gray-200 shadow-sm"):
            with ui.row().classes("w-full justify-between items-center mb-2"):
                ui.label("Editor de Prompts").classes(
            ...
            with ui.row().classes("w-full gap-4 items-center"):
                search_input = (
                    ...
                    .props('outlined dense prepend-inner-icon="search"')
                    .classes("flex-grow")
                )

                # Extract unique tasks and phases for filters
                unique_tasks = sorted(
                    list(set(m["task"] for m in PROMPT_METADATA.values()))
                )
                unique_phases = sorted(
                    list(set(m["phase"] for m in PROMPT_METADATA.values()))
                )

                task_filter = (
                    ui.select(
                        options=["Todas"] + unique_tasks,
                        value="Todas",
                        label="Tarea / Función",
                    )
        ...
        results_container = ui.grid().classes(
            "w-full grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3"
        )
        ...
        editor_card = (
            ui.card()
            .classes("w-full p-0 border border-gray-200 shadow-md hidden")
            ...
            with editor_content:
                ...
                with ui.row().classes("w-full items-center mb-6 pb-4 border-b gap-4"):
                    with ui.column().classes(f"p-3 bg-{meta['color']}-50 rounded-lg"):
                        ui.icon(meta["icon"], size="md").classes(
                            f"text-{meta['color']}-600"
                        )
                    with ui.column().classes("gap-1"):
                        ui.label(prompt_data.name).classes(
                            "text-2xl font-bold text-slate-800"
                        )
                        with ui.row().classes("gap-2"):
                            ui.badge(meta["task"], color=meta["color"]).props("outline")
                            ui.icon("arrow_right", size="xs").classes("text-gray-300")
                            ui.label(meta["phase"]).classes(
                                "text-sm text-gray-500 italic"
                            )

                # Metadata Row (Horizontal 3 columns)
                with ui.grid(columns=3).classes("w-full gap-4 mb-4"):
                    ui.input("ID Servicio", value=prompt_data.service_id).props(
                        'outlined dense readonly label-color="primary"'
                    ).classes("w-full")
                    ui.input(t("prompt_name"), value=prompt_data.name).props(
                        'outlined dense readonly label-color="primary"'
                    ).classes("w-full")
                    ui.textarea(t("description"), value=prompt_data.description).props(
                        'outlined dense readonly label-color="primary" rows=1'
                    ).classes("w-full")

                # Tier Selector
                with ui.column().classes(
                    "w-full mb-4 p-3 border border-gray-200 rounded-lg bg-gray-50"
                ):
                    with ui.row().classes("items-center gap-2 mb-2"):
                        ui.icon("layers", size="xs").classes("text-gray-500")
                        ui.label(t("admin.prompts.tier_selector_label")).classes(
                            "font-bold text-gray-700 text-sm"
                        )

                    # Generate options dict dynamically with translations
                    tier_opts = {}
                    default_tier_num = DEFAULT_TIER_MAPPING.get(
                        meta.get("task"), 2
                    ...
                    for opt in TIER_OPTIONS:
                        ...
                        if opt["value"] is None:
                            ...
                            default_tier_name = (
                                t(f"admin.prompts.tier_{default_tier_num}_label")
                                .split("-")[-1]
                                .strip()
                            )  # e.g. "Flash"
                    ...
                    tier_radio = (
                        ui.radio(
                            options=tier_opts,
                            value=prompt_data.tier_override
                            if hasattr(prompt_data, "tier_override")
                            else None,
                        )
                        .props("inline")
                    ...
                    # Tooltip dinámico
                    tier_info = ui.label("").classes(
                        "text-xs text-blue-600 italic mt-1 ml-1"
                    )

                    def update_tier_info(e):
                        selected = next(
                            (o for o in TIER_OPTIONS if o["value"] == e.value), None
                        )
                        if selected:
                ...
                with ui.column().classes("w-full flex-grow gap-2"):
                    with ui.row().classes("w-full justify-between items-center"):
                        ui.label(t("prompt_template")).classes(
                            "font-bold text-gray-700"
                        )
                        vars_label = ui.label(t("detected_vars") + ": -").classes(
                            "text-xs bg-gray-100 px-2 py-1 rounded text-gray-600"
                        )
                    ...
                    text_area = (
                        ...
                        .classes("w-full flex-grow text-sm font-mono")
                        .props(
                            'outlined input-class="font-mono text-gray-700 leading-relaxed" rows=20 spellcheck="false"'
                        )
                    )
                    ...
                    def update_vars(e):
                        ...
                        vars_label.text = (
                            f"{t('detected_vars')}: {', '.join(sorted(set(vs)))}"
                        )
                ...
                with ui.row().classes("w-full justify-end gap-3"):
                    ...
                    async def save_prompt():
                        if not current_selection["id"]:
                            return
                        async with AsyncSession(server_engine) as session:
                            p = await session.get(
                                ExtractionServiceConfig, current_selection["id"]
                            )
                            if p:
        ...
        def render_results():
            ...
            for p in current_prompts:
                ...
                # 2. Metadata Filters
                meta = PROMPT_METADATA.get(
                    p.name, {"task": "Sistema", "phase": "General"}
                )

                if sel_task != "Todas" and meta.get("task") != sel_task:
                    continue
                if sel_phase != "Todas" and meta.get("phase") != sel_phase:
                    continue
            ...
            with results_container:
                if not filtered:
                    ui.label("No se encontraron prompts.").classes(
                        "col-span-full text-center text-gray-400 italic py-4"
                    )
                ...
                for p in filtered:
                    ...
                    with (
                        ui.card()
                        .classes(
                            "p-3 cursor-pointer hover:shadow-lg transition-all border-l-4"
                        )
                        .style(f"border-left-color: var(--q-{meta['color']})")
                        .on("click", lambda _, i=p.service_id: load_prompt_editor(i))
                    ):
                        with ui.row().classes("items-start gap-3 no-wrap"):
                            ui.icon(meta["icon"]).classes(
                                f"text-{meta['color']}-500 mt-1"
                            )
                            with ui.column().classes("gap-1 flex-1 min-w-0"):
                                # Use text-xs for description info to fit better
                                ui.label(p.name.replace("System: ", "")).classes(
                                    "font-bold text-sm text-slate-700 leading-tight truncate w-full"
                                )
                                with ui.row().classes("gap-1 items-center"):
                                    # TIER BADGE
                                    tier_val = getattr(p, "tier_override", None)
                                    badge, _badge_color = "⚪", "gray"
                                    for opt in TIER_OPTIONS:
                                    ...
                                    ui.label(badge).classes("text-xs").tooltip(
                                        t(
                                            "admin.prompts.tier_badge_tooltip",
                                            tier=(tier_val if tier_val else "Default"),
                                        )
                                    )
                                    ui.label(meta["task"]).classes(
