"""
[LEGACY] atoms_page.py - UI Antigua de Administración de Acciones.
ESTE ARCHIVO ESTÁ DEPRECIADO. Por favor, utiliza `script_library_page.py` 
para el catálogo de acciones unificado.
Se mantiene únicamente por retrocompatibilidad y referencia histórica.
"""
import json
...
class AtomsPageState:
    """
    Mantiene el estado reactivo de la página de administración de acciones.
...
async def load_atoms(page_state: AtomsPageState):
    ...
    # Cargar conteo de usos para cada acción
...
def atoms_page_content():
    """
    Renderiza la interfaz principal para la gestión del catálogo de acciones.
    Permite visualizar, filtrar, editar y crear nuevos bloques de construcción
    reutilizables para los flujos de trabajo.
    """
    t = state.i18n.t
    page_state = AtomsPageState()

    # Estado del modo de la página: 'list' o 'gallery'
    ...
    def show_gallery():
        """Muestra la galería de tipos de acciones en el cuerpo central y abre el drawer lateral."""
        # Registrar callback para que la galería del drawer también pueda navegar
        async def standalone_atom_select(step_type: StepType, subtype=None, script_id=None, **kwargs):
            handle_atom_type_selected(step_type)
            return True  # Indica que ya se ha gestionado la navegación

        state.on_atom_select_callback = standalone_atom_select
    ...
    def show_list():
        """Vuelve a mostrar la lista de acciones y cierra el drawer lateral."""
        state.on_atom_select_callback = None  # Limpiar callback standalone
        layout_manager.exit_focus_mode()
        page_mode['current'] = 'list'
        render_page_content.refresh()

    def handle_atom_type_selected(step_type: StepType):
        """Navega a la página de diseño del tipo de acción seleccionado."""
        from client_app.app.config.atom_catalog import STEP_TYPE_ROUTES
        route = STEP_TYPE_ROUTES.get(step_type)
        if route:
            ui.navigate.to(route)
        else:
            # Para acciones sin página dedicada, mostrar mensaje
            ui.notify(f'El tipo {step_type.value} aún no tiene página de diseño', type='warning')
    ...
    def render_gallery_mode():
        """Renderiza la galería de tipos de acciones para crear una nueva."""
        from client_app.app.config.atom_catalog import (
        ...
        # Descripción
        ui.label(t('atoms.select_type_hint', 'Elige el tipo de acción que deseas crear. Serás redirigido a la página de diseño correspondiente.')).classes('text-gray-600 mb-6')
    ...
    def render_filters():
        ...
        with ui.card().classes('w-full p-4 mb-4'):
            with ui.row().classes('w-full items-center gap-4 flex-wrap'):
                ...
                ui.checkbox(
                    t('atoms.filter_user_only', 'Solo míos'),
                    value=False,
    ...
    def render_atom_card(atom: AtomRegistry, page_state: AtomsPageState, refresh_callback):
        ...
        with ui.card().classes('w-72 p-4 hover:shadow-lg transition-shadow'):
            ...
            with ui.row().classes('w-full justify-end gap-1'):
                ...
                # Eliminar (solo si no tiene usos)
                if usage_count == 0:
                    ui.button(
                        icon='delete',
                        color='red',
...
def show_atom_detail_dialog(atom: AtomRegistry):
    """
    Muestra un cuadro de diálogo modal con la información técnica detallada de una acción.
    Incluye entradas, salidas, configuración y metadatos de seguridad.
    """
    t = state.i18n.t
...
async def show_edit_atom_dialog(atom: AtomRegistry, refresh_callback):
    """Abre diálogo de edición según el estado de la acción."""
    t = state.i18n.t
...
async def _show_full_edit_dialog(atom: AtomRegistry, refresh_callback):
    """Modal para edición completa de átomo (modo DRAFT)."""
    t = state.i18n.t
...
async def show_maintenance_dialog(atom: AtomRegistry, refresh_callback):
    """Abre el DrawerHub en modo documentación para edición de metadatos."""
...
async def create_atom_version_and_edit(atom_id, dialog, refresh_callback):
    try:
        ...
        ui.notify(f"Nueva versión {new_atom.version} creada como Borrador", type='positive')
        dialog.close()
...
def show_duplicate_dialog(atom: AtomRegistry, refresh_callback):
    """Muestra diálogo para duplicar acción."""
    t = state.i18n.t
