"""
Atoms Page - UI de Administraci├│n de ├ütomos.
Prompt 6.2 del plan de refactorizaci├│n del editor de flujos.

Permite gestionar el cat├ílogo de ├ítomos reutilizables:
- Lista con filtros (tipo, m├¡os, sistema)
- Ver detalle, editar, duplicar, eliminar
- Crear nuevos ├ítomos personalizados
"""
import json
...
class AtomsPageState:
    """
    Mantiene el estado reactivo de la p├ígina de administraci├│n de ├ítomos.
    Incluye la lista de ├ítomos cargados, filtros activos y estados de carga.
    """
    def __init__(self):
...
async def load_atoms(page_state: AtomsPageState):
    """Carga los ├ítomos con los filtros aplicados."""
    ...
    # Cargar conteo de usos para cada ├ítomo
...
def atoms_page_content():
    """
    Renderiza la interfaz principal para la gesti├│n del cat├ílogo de ├ítomos.
    ...
    def show_gallery():
        """Muestra la galer├¡a de tipos de ├ítomos en el cuerpo central."""
        page_mode['current'] = 'gallery'
        render_page_content.refresh()

    def show_list():
        """Vuelve a mostrar la lista de ├ítomos."""
        page_mode['current'] = 'list'
        render_page_content.refresh()

    def handle_atom_type_selected(step_type: StepType):
        """Navega a la p├ígina de dise├▒o del tipo de ├ítomo seleccionado."""
        from client_app.app.config.atom_catalog import STEP_TYPE_ROUTES
        route = STEP_TYPE_ROUTES.get(step_type)
        if route:
            ui.navigate.to(route)
        else:
            # Para ├ítomos sin p├ígina dedicada, mostrar mensaje
    ...
    def render_gallery_mode():
        """Renderiza la galer├¡a de tipos de ├ítomos para crear uno nuevo."""
        from client_app.app.config.atom_catalog import (
        ...
        with ui.row().classes('w-full items-center justify-between mb-4'):
            with ui.row().classes('items-center gap-2'):
                ...
                ui.label(t('atoms.select_type', 'Selecciona el tipo de ├ítomo')).classes('text-2xl font-bold')

        # Descripci├│n
        ui.label(t('atoms.select_type_hint', 'Elige el tipo de ├ítomo que deseas crear. Ser├ís redirigido a la p├ígina de dise├▒o correspondiente.')).classes('text-gray-600 mb-6')
    ...
    def render_list_mode():
        """Renderiza la lista de ├ítomos existentes."""
        # Header
        with ui.row().classes('w-full items-center justify-between mb-4'):
            ui.label(t('atoms.title', 'Cat├ílogo de ├ítomos')).classes('text-2xl font-bold')
            ui.button(
                t('atoms.new', 'Nuevo ├ítomo'),
                icon='add',
    ...
    def render_filters():
        """Renderiza los filtros de la lista de ├ítomos."""
    ...
    def render_atoms_list():
        if page_state.is_loading:
            with ui.row().classes('w-full justify-center p-8'):
                ui.spinner(size='lg')
                ui.label(t('atoms.loading', 'Cargando ├ítomos...'))
            return

        if not page_state.atoms:
            with ui.card().classes('w-full p-8 text-center'):
                ui.icon('inventory_2', size='xl').classes('text-gray-400')
                ui.label(t('atoms.empty', 'No hay ├ítomos disponibles')).classes('text-gray-500 text-lg mt-2')
                ui.label(t('atoms.empty_hint', 'Crea tu primer ├ítomo personalizado')).classes('text-gray-400')
            return
    ...
    def render_atom_card(atom: AtomRegistry, page_state: AtomsPageState, refresh_callback):
        ...
        with ui.card().classes('w-72 p-4 hover:shadow-lg transition-shadow'):
            ...
            # Description
            desc = atom.description or t('atoms.no_description', 'Sin descripci├│n')
            ...
            with ui.row().classes('w-full justify-end gap-1'):
                ...
                ).props('flat round dense').tooltip(t('atoms.view', 'Ver detalle'))
                ...
                if not atom.is_system:
                    ...
                    ).props('flat round dense').tooltip(t('atoms.edit', 'Editar'))

                # Duplicar
                ui.button(
                    icon='content_copy',
                    on_click=lambda a=atom: show_duplicate_dialog(a, refresh_callback)
                ).props('flat round dense').tooltip(t('atoms.duplicate', 'Duplicar'))
                ...
                if not atom.is_system and usage_count == 0:
                    ...
                    ).props('flat round dense').tooltip(t('atoms.delete', 'Eliminar'))
...
def show_atom_detail_dialog(atom: AtomRegistry):
    """
    Muestra un cuadro de di├ílogo modal con la informaci├│n t├®cnica detallada de un ├ítomo.
    ...
    with ui.dialog() as dialog, ui.card().classes('w-[600px] max-h-[80vh]'):
        ...
        # Config Schema
        with ui.expansion(t('atoms.config_schema', 'Esquema de Configuraci├│n'), icon='schema').classes('w-full'):
            try:
                schema = json.loads(atom.config_schema) if atom.config_schema else {}
                ui.json_editor({'content': {'json': schema}}).classes('h-48')
            except json.JSONDecodeError:
                ui.label('Error al parsear schema').classes('text-red-500')

        # Default Config
        with ui.expansion(t('atoms.default_config', 'Configuraci├│n por Defecto'), icon='settings').classes('w-full'):
            try:
                default = json.loads(atom.default_config) if atom.default_config else {}
                ui.json_editor({'content': {'json': default}}).classes('h-48')
            except json.JSONDecodeError:
                ui.label('Error al parsear config').classes('text-red-500')

        with ui.row().classes('w-full justify-end mt-4'):
            ui.button(t('common.close', 'Cerrar'), on_click=dialog.close)

    dialog.open()


async def show_edit_atom_dialog(atom: AtomRegistry, refresh_callback):
    """Abre di├ílogo de edici├│n seg├║n el estado del ├ítomo."""
    t = state.i18n.t
...
async def _show_full_edit_dialog(atom: AtomRegistry, refresh_callback):
    ...
    async def save_changes():
        try:
            ...
            ui.notify(t('atoms.updated', '├ütomo actualizado'), type='positive')
            dialog.close()
    ...
    with ui.dialog() as dialog, ui.card().classes('w-[600px]'):
        with ui.row().classes('w-full items-center justify-between mb-4'):
            ui.label(t('atoms.edit_title', 'Editar ├ítomo (borrador)')).classes('text-xl font-bold')
            ui.button(icon='close', on_click=dialog.close).props('flat round')
        ...
        ui.textarea(
            label=t('atoms.description', 'Descripci├│n'),
            value=form_data['description'],
            on_change=lambda e: form_data.update({'description': e.value})
        ).classes('w-full')

        ui.input(
            label=t('atoms.version', 'Versi├│n'),
            value=form_data['version'],
            on_change=lambda e: form_data.update({'version': e.value})
        ).classes('w-48')

        with ui.expansion(t('atoms.config_schema', 'Esquema de Configuraci├│n (JSON)'), icon='schema').classes('w-full'):
            ui.textarea(
        ...
        with ui.expansion(t('atoms.default_config', 'Config por Defecto (JSON)'), icon='settings').classes('w-full'):
            ui.textarea(
...
def show_duplicate_dialog(atom: AtomRegistry, refresh_callback):
    """Muestra di├ílogo para duplicar ├ítomo."""
    t = state.i18n.t
    ...
    async def do_duplicate():
        try:
            ...
            ui.notify(t('atoms.duplicated', '├ütomo duplicado'), type='positive')
            dialog.close()
            await refresh_callback()
        except ValueError as e:
            ui.notify(str(e), type='negative')
    ...
    with ui.dialog() as dialog, ui.card().classes('w-96'):
        ui.label(t('atoms.duplicate_title', 'Duplicar ├ítomo')).classes('text-lg font-bold mb-4')
        ui.label(t('atoms.duplicate_hint', 'Introduce el nombre para la copia:')).classes('text-gray-600')

        ui.input(
            label=t('atoms.new_name', 'Nuevo nombre'),
...
def show_delete_dialog(atom: AtomRegistry, refresh_callback):
    ...
    async def do_delete():
        ...
        if success:
            ui.notify(t('atoms.deleted', '├ütomo eliminado'), type='positive')
            dialog.close()
            await refresh_callback()
        else:
            ui.notify(t('atoms.delete_error', 'Error al eliminar'), type='negative')

    with ui.dialog() as dialog, ui.card().classes('w-96'):
        ui.label(t('atoms.delete_title', 'Eliminar ├ítomo')).classes('text-lg font-bold mb-4')
        ui.label(
            t('atoms.delete_confirm', f'┬┐Seguro que deseas eliminar "{atom.name}"?')
        ).classes('text-gray-600')
        ui.label(
            t('atoms.delete_warning', 'Esta acci├│n no se puede deshacer.')
...
def show_atom_wizard(initial_step_type: StepType, refresh_callback):
    """
    Despliega el asistente (wizard) para la creaci├│n de un nuevo ├ítomo.
    Gu├¡a al usuario a trav├®s del proceso de configuraci├│n, definici├│n de datos
    y guardado en el cat├ílogo.

    Args:
        initial_step_type: El tipo de ├ítomo seleccionado inicialmente.
        refresh_callback: Funci├│n a llamar para refrescar la lista de ├ítomos al finalizar.
    """
    t = state.i18n.t

    form_data = {
        'name': '',
        'description': t('atoms.default_desc', '├ütomo personalizado'),
    ...
    async def create_atom():
        try:
            ...
            ui.notify(t('atoms.created', '├ütomo creado exitosamente'), type='positive')
            dialog.close()
            await refresh_callback()
        except ValueError as e:
            ui.notify(str(e), type='negative')
    ...
    with ui.dialog() as dialog, ui.card().classes('w-[80vw] max-w-5xl h-[70vh] flex flex-col p-0 overflow-hidden'):
        ...
        with ui.row().classes('w-full items-center justify-between p-4 bg-slate-50 border-b shrink-0'):
            ...
            ui.label(t('atoms.create_title', f'Crear nuevo ├ítomo: {op_label}')).classes('text-xl font-bold text-slate-800')
        ...
        def render_wizard():
            ...
            with ui.row().classes('w-full justify-center gap-4 mb-6 border-b pb-4 pt-4 bg-white'):
                for i, label in enumerate(['Tipo', 'Informaci├│n', 'Configuraci├│n'], 1):
            ...
            if wizard_step['current'] == 2:
                with ui.column().classes('w-full h-full p-6'):
                    ui.label(t('atoms.step2_title', 'Informaci├│n del ├ítomo')).classes('text-lg font-bold mb-4')
                    ...
                    with ui.row().classes('items-center gap-4 mb-6'):
                         ...
                         with ui.column().classes('gap-0'):
                             ...
                             ui.button('Cambiar Tipo', on_click=dialog.close).props('flat dense color=primary size=sm').classes('-ml-2')
            ...
            # Step 3: Configuraci├│n
                ...
                with ui.column().classes('w-full h-full p-6'):
                    ui.label(t('atoms.step3_title', 'Esquema de configuraci├│n')).classes('text-lg font-bold mb-4')

                    ui.label(
                        t('atoms.schema_hint', 'Define los campos que el usuario deber├í configurar al usar este ├ítomo.')
                    ).classes('text-gray-600 mb-4')

                    with ui.expansion('Esquema JSON (avanzado)', icon='code').classes('w-full mb-4'):
                        ui.textarea(
                            value=form_data['config_schema'],
                            on_change=lambda e: form_data.update({'config_schema': e.value})
                        ).classes('w-full font-mono').props('rows=8 outlined')

                    with ui.expansion('Valores por defecto (JSON)', icon='settings').classes('w-full'):
                        ui.textarea(
                            value=form_data['default_config'],
                            on_change=lambda e: form_data.update({'default_config': e.value})
