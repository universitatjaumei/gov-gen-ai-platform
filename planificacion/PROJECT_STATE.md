...
| **Bloque REV** — Primer vistazo del usuario a Plataforma (correcciones de bajo riesgo) | REV.5 ✅ | — | — | ✅ **Completo (2026-08-23)** — los 5 prompts, agrupados a petición del usuario tras su primera pasada por el módulo Plataforma. REV.1 guardarraíl del DSN del entorno + borrado de las 97 filas `ada-*` que el test de SAML dejó en la BD de desarrollo; REV.2 valores por defecto de recuperación editables (el `PATCH` ya lo aceptaba desde PLAT.3, faltaba el control); REV.3 la opción activa del menú con negrita y barra en vez de recuadro, en los tres menús; REV.4 el selector del logotipo accionable y sin exigir tema previo; REV.5 el enlace de la cola de vigencia, que llevaba a Informes por dos fallos encadenados. **Suites completas: frontend 592 passed, 0 failed (`--no-file-parallelism`); `tsc --noEmit` limpio.** Sin migraciones y sin cambios de contrato. **Verificado en navegador contra el servidor real** los cuatro prompts de UI, con los artefactos de verificación retirados. **Lo que este bloque NO toca**, y sigue abierto del mismo repaso: la cascada de color no llega al panel (sólo el logotipo la consume), Personas sin borrado ni superadministrador visible ni organización, buscador en los prompts de actividad, selector global de organización, validar la vigencia desde la pantalla —no existe endpoint— y la estrategia de datos del piloto |
