...
**Cursor actual: Bloque DOM / DOM.5.** El 2026-09-02 apareció el registro A de `normativa.uji.es`
y con él se desbloqueó el **Bloque DOM**, que estaba pendiente de ese prerrequisito externo desde
el 2026-09-01. Se rehizo antes de ejecutarlo —de 4 prompts a 5— porque el usuario pidió que la
raíz del dominio sea la portada pública y el login viva en `/panel/`, que es justo lo que la
primera versión había decidido no hacer. DOM.1–DOM.4 ✅ el mismo día: el reparto en Caddy, el
panel bajo `/panel/`, las variables del repositorio al dominio y el sitio republicado. **Modelo sugerido para el próximo prompt: Sonnet** (DOM.5 despliega y verifica
en el navegador). Lo anterior:
