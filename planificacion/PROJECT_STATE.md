...
| **Bloque ACT** — Actualizar el corpus sin pagar dos veces; idioma, emparejamiento y vigencia como el corpus los declara | — | **ACT.0** (corpus: externas fuera de `normatiu` en `assistents.json`) y **ACT.1** (idempotencia del reconciliador) | **Sonnet** para ACT.0–ACT.2 y ACT.5–ACT.6; **Opus** para ACT.3 (canónica por lengua) y ACT.4 (vigencia) | ⏳ **Planificado el 2026-08-28**, tras auditar código y configuración con el corpus del 27-08 en `--dry-run` (nada ingerido). Hallazgos: la exclusión de las 22 externas vive sólo en la BD y el corpus la deshace; `canonica` es filtro duro y con este corpus 57 normas castellanas dejarían de responderse en castellano; `ca`≠`val` en todo el sistema; `vigencia_curs` no existe en el repo; 292 «actualizaciones» falsas por pasada por fechas *aware/naive* y el reconciliador pisa la validación del panel; `chunk_copier` (HIB.N) sin llamadores. Siete prompts en `Plan_TDD_Fase1.md` §Bloque ACT; decisiones para SG en `docs/CONFIGURACION_APERTURA_PILOTO.html` §12. **El usuario decidió no ingerir hasta cerrar el bloque.** |
