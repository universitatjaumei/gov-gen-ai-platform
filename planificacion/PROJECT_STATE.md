...
| **Bloque IMG** — Que CI construya la imagen y la arranque | IMG.1 ✅ (2026-09-17), **ejecutado en CI** | — (falta verlo rojo con el defecto dentro) | Sonnet | ▶ **IMG.1 hecho, medido y en verde en CI el 2026-09-17** (run `35214706322`). Nace el 2026-09-15, el mismo día en que el hueco tumbó producción 35 minutos: **el conjunto de dependencias que se despliega —base, sin extras— era el único que no probaba nadie**. El job `imagen` construye las cuatro imágenes con los `Dockerfile` del despliegue, migra con la recién construida y **la arranca** hasta `/health` 200, con `ENVIRONMENT=production`; el cliente de la API lo hereda del job `contract` en vez de reinstalar uv y node. Guardarraíl: `test_img1_ci_construye_y_arranca_la_imagen.py`, que **lee de `deploy.yml`** qué imágenes y qué `Dockerfile` usa el despliegue en vez de copiarlos. **Cinco mutaciones comprobadas una a una, y la cuarta se quedó VERDE en la primera versión**: la comprobación preguntaba si había un `docker run` y el paso de migraciones ya ejecuta uno, así que quitar el arranque de la aplicación no se notaba — ahora pregunta por `docker run -d` de una etiqueta construida aquí. **Tiempo real: 2 min 45 s** (construir las cuatro 111 s, migrar 4 s, arrancar y servir 12 s). **La medida local sobreestimaba por casi el doble** (5,6 min) y con ella se escribió que el job sería el camino crítico: **no lo es** — `contract` 3,33 + `imagen` 2,75 = 6,1 min frente a los 7,87 de `Lint & Test`, así que **sale gratis en reloj de pared**. Corre en cada push, y además acotarlo a `main` no protegería porque `deploy.yml` dispara con ese mismo push y en paralelo. Detalle en `fase1/75_BLOQUE_IMG.md` |
