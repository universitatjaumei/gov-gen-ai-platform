...
| **Bloque IMG** — Que CI construya la imagen y la arranque | IMG.1 ✅ (2026-09-17), **medido** | — (falta verlo rojo en CI) | Sonnet | ▶ **IMG.1 hecho y medido el 2026-09-17**; falta su primera ejecución en CI. Nace el 2026-09-15, el mismo día en que el hueco tumbó producción 35 minutos: **el conjunto de dependencias que se despliega —base, sin extras— era el único que no probaba nadie**. El job `imagen` construye las cuatro imágenes con los `Dockerfile` del despliegue, migra con la recién construida y **la arranca** hasta `/health` 200, con `ENVIRONMENT=production`; el cliente de la API lo hereda del job `contract` en vez de reinstalar uv y node. Guardarraíl: `test_img1_ci_construye_y_arranca_la_imagen.py`, que **lee de `deploy.yml`** qué imágenes y qué `Dockerfile` usa el despliegue en vez de copiarlos. **Cinco mutaciones comprobadas una a una, y la cuarta se quedó VERDE en la primera versión**: la comprobación preguntaba si había un `docker run` y el paso de migraciones ya ejecuta uno, así que quitar el arranque de la aplicación no se notaba — ahora pregunta por `docker run -d` de una etiqueta construida aquí. **Medido en local** (Docker Desktop, en frío, Postgres desechable en su propia red): construir las cuatro **309 s** (app 166, frontend 66, sandbox 59, mcp 18), migrar **6 s**, arrancar hasta `/health` **~20 s** — **≈ 5,6 min**, con el registro del contenedor en **0 errores y 0 avisos**. Contra los tiempos reales de CI (`Lint & Test` 6,28 min marca el total), el job **pasaría a ser el camino crítico** y añadiría 2-3 min. **Se acepta y corre siempre**: acotarlo a `main` no protege, porque `deploy.yml` dispara con el mismo push y en paralelo. Detalle en `fase1/75_BLOQUE_IMG.md` |
...
**Cursor: Bloque IMG ▶ — IMG.1 escrito y verde el 2026-09-17, a falta de su primera ejecución
en CI.** El job `imagen` construye las cuatro imágenes con los `Dockerfile` del despliegue,
...
el bloque—. **Medido en local el mismo día**, con Docker ya arrancado: **≈ 5,6 min** la secuencia
entera (309 s las cuatro imágenes, 6 s migrar, ~20 s hasta `/health` 200) y el registro del
contenedor con **0 errores y 0 avisos**. Con eso el job **pasa a ser el camino crítico de CI** y
añade 2-3 minutos: **se acepta y corre en cada push**, porque acotarlo a `main` no protege —
`deploy.yml` dispara con ese mismo push y en paralelo—. Lo que queda es verlo rojo en una
ejecución real. Detalle en [`fase1/75_BLOQUE_IMG.md`](fase1/75_BLOQUE_IMG.md).
