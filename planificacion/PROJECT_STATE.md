...
**Cursor actual: Bloque USR, prompt USR.9.** El usuario pidió el 2026-09-03, antes de seguir con
LANG, **arreglar los tres hallazgos** que el informe de cierre reportaba sin arreglar: USR.8 ✅ (el
chat agéntico que no terminaba nunca), USR.9 (la pantalla de Personas es sólo de superadmin, así
que la capacidad que USR.1 dio al administrador **no se alcanza desde el panel**) y USR.10 (quien
no tiene módulos aterriza en una página que además se le enseña, con el texto equivocado, a quien
sí los tiene pero no ése). Los **7 prompts planificados** están cerrados: una persona de `hub_users` puede tener contraseña local, entrar con su rol real, que se
...
sólo entonces retirar los seis `SuperAdminAccount`.
