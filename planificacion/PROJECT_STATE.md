...
**Cursor: Bloque DEP ▶ en curso (2026-09-10), 4 de 7 hechos.** DEP.1, DEP.2, DEP.5 y DEP.6
cerrados; **DEP.3 y DEP.4 revertidos y esperando decisión del usuario**, y DEP.7 depende de
ellos. **REPO sigue abierto y bloqueado** en que le den rol `admin` sobre el repositorio ya
transferido a `universitatjaumei`. Después **DEP → PLG → DIN → FUN**, con PRC en paralelo y a tandas.
...
| 7 ▶ | **DEP** | 7 (4 ✅) | **Parcialmente hecho el 2026-09-10: DEP.1, DEP.2, DEP.5 y DEP.6 ✅.** El árbol del servidor pasa de **408 a 199 dependencias** y el de producción del frontend de **21 avisos (13 altos) a cero**. **Quedan tres, y dos esperan criterio del usuario**: **DEP.3** (`starlette` 1.6 deja de aplanar las rutas en `app.routes` y mete un `_IncludedRouter` sin API pública — la app funciona y el contrato OpenAPI sale idéntico, pero 10 tests en 8 ficheros enumeran así, y leerlo del esquema los volvería vacuos); **DEP.4** (`camelot-py` 2.0 es la única vía para quitar los 30 avisos de `pypdf`, y cambia la extracción en **2 de 20 PDF reales**, con una tabla de 28 filas que desaparece); y **DEP.7**, que no puede cerrarse hasta que esos dos se decidan. **Lo que DEP.1 destapó y no estaba previsto**: al sacar `browser-use` y `ragas` aparecieron **tres paquetes usados y no declarados** —`python-multipart`, `langchain`, `beautifulsoup4`— más `langchain-community`, que sostiene Ollama. Entra un guardarraíl que los caza todos de golpe |
