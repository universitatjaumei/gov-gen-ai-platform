...
**Cursor actual: Bloque USR, prompt USR.10.** El usuario pidió el 2026-09-03, antes de seguir con
LANG, **arreglar los tres hallazgos** que el informe de cierre reportaba sin arreglar: USR.8 ✅ el
chat agéntico que no terminaba nunca ni avisaba, USR.9 ✅ la pantalla de Personas fuera del módulo
de plataforma y con su listado abierto a quien administra una organización, y USR.10 el «no tienes
ningún módulo» que se le enseña a quien sí los tiene. Lo que el bloque dejó hecho: una persona de `hub_users` puede tener contraseña local, entrar con su rol real, que se
...
**Siguiente por el orden acordado el 2026-09-02: Bloque LANG** (2 prompts, Sonnet). Después REG →
VAS → NIC → REPO → PLG → DIN → FUN, con PRC en paralelo y a tandas. Lo anterior:
...
|---|---|---|---|
| ~~1~~ ▶ | ~~**USR**~~ (+3 arreglos) | 7+3 | **Cierra una exposición viva**: en producción hay siete superadministradores, seis de ellos probadores del piloto, con **la misma contraseña** y sin poder cambiarla ellos mismos. Y el piloto ya está abierto mientras el IdP no tiene fecha |
