...
**Cursor actual: sin bloque en curso — el siguiente lo elige el usuario.** El **Bloque DOM ✅
quedó completo el 2026-09-02** (5 prompts), el mismo día que apareció el registro A de
`normativa.uji.es`: el dominio institucional sirve la portada pública en la raíz, el cercador, las
fichas y la API, y el panel vive en `/panel/` con su login. El nombre provisional sigue sirviendo
lo mismo y **no se retira todavía** (apuntan a él el paso de comprobación de `deploy.yml`, la
vigilancia de tiempo de actividad y los enlaces ya enviados; recomendado un mes). Reparto completo
en `docs/DESPLIEGUE_PROTOTIPO_GCP.md` §3.sexies. **Pendiente de una persona**:
`pruebas_manuales/pruebas_manuales_bloqueDOM.bat` —el texto de presentación de la portada, que es
texto institucional público y nuevo— y la decisión sobre el provisional. **Bloques pendientes y
sin orden decidido**: NIC, REPO, REG, VAS, DIN, USR, LANG, PLG, FUN, PRC. Lo anterior:
