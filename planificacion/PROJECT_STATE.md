...
## 👉 EMPEZAR AQUÍ EL PRÓXIMO DÍA (actualizado 2026-09-14, al poner DEP.4 al día y unificar el cursor)
...
**Cursor: Bloque DEP ▶ en curso, 5 de 7.** DEP.1, DEP.2, DEP.4, DEP.5 y DEP.6 cerrados el
2026-09-10. Quedan dos:

- **DEP.3** — `starlette` 0.50 → 1.x. **Revertido y esperando decisión del usuario**: la app
  funciona y el contrato OpenAPI sale idéntico, pero 1.6 deja de aplanar las rutas en `app.routes`
  y mete un `_IncludedRouter` sin API pública, y **10 tests en 8 ficheros enumeran así**. Leerlos
  del esquema los volvería vacuos, que es la razón de que la decisión no sea del agente.
- **DEP.7** — encender la puerta de CI (de informar a bloquear). **Depende de DEP.3**: el umbral
  se fija con la medida final encima de la mesa, no antes.
...
**Sin desplegar**: `origin/main` está en `2518a46` (2026-09-04) y `desarrollo` lleva **25 commits**
por delante —LANG entero y todo DEP—. El paso a `main` es decisión del usuario. LANG no cambia
comportamiento en producción porque el modo `prefer` es el que producción necesita.
...
| 7 ▶ | **DEP** | 7 (5 ✅) | **Parcialmente hecho el 2026-09-10: DEP.1, DEP.2, DEP.4, DEP.5 y DEP.6 ✅.** El árbol del servidor pasa de **408 a 197 dependencias** y el de producción del frontend de **21 avisos (13 altos) a cero**. **DEP.4 se cerró midiendo, no confiando**: `camelot-py` 2.0 saca `pypdf` del árbol entero, y de los **2 PDF de 20** cuya extracción cambia, mirados celda a celda, **en los dos la versión vieja era la equivocada** —una cabecera de dos niveles colapsada y una portada de 28×2 que no era tabla, sino un falso positivo que la 2.0 rechaza bien—. **Quedan dos, y el primero espera criterio del usuario**: **DEP.3** (`starlette` 1.6 deja de aplanar las rutas en `app.routes` y mete un `_IncludedRouter` sin API pública — la app funciona y el contrato OpenAPI sale idéntico, pero 10 tests en 8 ficheros enumeran así, y leerlo del esquema los volvería vacuos); y **DEP.7**, que no puede cerrarse hasta que DEP.3 se decida. **Lo que DEP.1 destapó y no estaba previsto**: al sacar `browser-use` y `ragas` aparecieron **tres paquetes usados y no declarados** —`python-multipart`, `langchain`, `beautifulsoup4`— más `langchain-community`, que sostiene Ollama. Entra un guardarraíl que los caza todos de golpe |
