...
**Deploy (D.0–D.6) → pruebas humanas y RAG.6b → REV → NIC → REPO.** (**PLAT** e **IDE** se planificaron el 2026-08-22 y su sitio en esta secuencia está sin decidir: la propuesta es entre NIC y REPO, IDE antes que PLAT porque **IDE.1 arregla algo que hoy impide administrar** —el rol puesto a mano lo pisa el siguiente inicio de sesión por SSO—, y con PLAT.1+PLAT.2 adelantables si el piloto tiene administradores que no sean superadmin)
...
**Deploy → pruebas humanas y RAG.6b → REV → NIC → NIC.5 (el usuario borra `_legacy_nicegui/`) → REPO.** (**PLAT** e **IDE** se planificaron el 2026-08-22 y su sitio en esta secuencia está sin decidir: la propuesta es entre NIC y REPO, IDE antes que PLAT porque **IDE.1 arregla algo que hoy impide administrar** —el rol puesto a mano lo pisa el siguiente inicio de sesión por SSO—, y con PLAT.1+PLAT.2 adelantables si el piloto tiene administradores que no sean superadmin)
