...
| 6 ▶ | **REPO** | 5 (2 ✅) | **En curso.** REPO.3 ✅ y REPO.5 ✅ el 2026-09-07. Quedan **REPO.1 → REPO.4 → REPO.2**, y sólo REPO.4 espera a la organización: se puede seguir con otros bloques sin que REPO se quede obsoleto. REPO.1 lleva ahora el **reparto de `docs/`** y el **filtro del historial**, porque la visibilidad alcanza a los commits y borrar antes de abrir no saca nada del pasado. **Sin repositorio privado**: se propuso uno para 15 ficheros y el usuario lo cuestionó con razón —un archivo de instantáneas no es un repositorio de operación, y tres de esos ficheros estaban clasificados por su nombre y son publicables—. Salen del árbol 15 rutas a `_local/` + Drive: los once tableros (por decisión institucional, no por impedimento técnico), el trío de marca y la revisión de literatura. **La ventana sigue abierta y medida**: el commit huérfano `dc4904e0c763` todavía sirve `logs/` con 46 ficheros, 17 días después de la reescritura. Se hace **antes** de que exista el primer *fork* y antes de abrir. REPO.1 y REPO.2 los ejecuta el usuario |
