...
class HubLexiconPair(HubConfigBase):
    ...
    **Se proyecta sobre `bilingual_terms`, que alimenta el `tsvector` como columna generada.** O
    ...
    # MT.1 — el vocabulario de una organización no puede cambiar las búsquedas de otra.
    __ambito__ = Ambito.ORGANIZACION
    __table_args__ = (
        UniqueConstraint(
            "organizacion_id",
        ...
        # mismo criterio que `origen` en `HubUser`. Lo que no puede llevarlo son los términos.
        CheckConstraint(
    ...
    #:
    #: Sin FK a `hub_documents`: es operacional y esta tabla es de configuración, y la frontera
    #: edge-cloud prohíbe cruzar las dos bases. Se consulta por id, como el resto.
