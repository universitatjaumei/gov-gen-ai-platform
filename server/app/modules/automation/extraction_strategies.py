...
async def analyze_document_structure(doc_text_a: str, doc_text_b: str, language_hint: str, config: Dict[str, Any], system_prompt: str = None) -> Dict[str, Any]:
    """
...
"""
    ...
    resultado = await ejecutar_tarea(prompt_final, config, script_origen="brain.extraction_strategies")
...
def _safe_json_parse(text: str) -> Dict[str, Any]:
    ...
    """
    import json, re
    if not text: return {"fields": [], "format_ok": False}
            ...
            if isinstance(raw_fields, list):
                for item in raw_fields:
                    if isinstance(item, dict):
                         ...
                         try: page_hint = int(ph) if ph is not None else None
                         except: page_hint = None
...
async def extraer_datos_precision(texto_fitz: str, texto_plumber: str, campos_seleccionados: List[str], definicion_usuario: str, config: Dict[str, Any], system_prompt: str, usar_tier_2: bool = False, ejemplos_validacion: Optional[Dict] = None) -> Dict[str, Any]:
    """
    ...
    if config_rol is None: config_rol = {}
    ...
    resultado = await ejecutar_tarea(prompt_extraccion, config_rol, script_origen="brain.extraction_strategies")

    if "response" in resultado:
        datos_limpios = limpiar_respuesta_json(resultado["response"])

        if isinstance(datos_limpios, dict) and "datos" not in datos_limpios:
            if all(isinstance(v, dict) and ("valor" in v or "pagina" in v) for v in datos_limpios.values()):
...
async def refinar_extraccion_con_feedback(texto_fitz: str, texto_plumber: str, datos_anteriores: Dict[str, Any], feedback_usuario: str, config: Dict[str, Any], system_prompt: str) -> Dict[str, Any]:
    """
    Fase 1 (Refinamiento): Re-extraccion inteligente guiada por correcciones del usuario (Prompt Inyectado).
    """
    config_rol = config.get('logico_navegacion', config) if isinstance(config, dict) and 'logico_navegacion' in config else config
    ...
    prompt_refinamiento = system_prompt \
        .replace("{texto_plumber}", texto_plumber[:45000]) \
    ...
    resultado = await ejecutar_tarea(prompt_refinamiento, config_rol, script_origen="brain.extraction_strategies")
...
async def generar_script_determinista(docs_text_list: List[Dict[str, str]], campos_objetivo: List[str], config: Dict[str, Any], system_prompt: str, valores_ejemplo: Optional[Dict] = None, info_descubrimiento: Optional[Dict] = None, feedback_usuario: str = None, tabla_auditoria: str = None, field_definitions: Optional[List[Dict]] = None) -> str:
    """
    ...
    if not system_prompt:
        raise ValueError("El prompt de sistema es obligatorio para la Generacion de Scripts.")
    ...
    if field_definitions:
        ...
        for fd in field_definitions:
             ...
             field_defs_str += f"  - {fd['name']}: {opt} '{desc}'. (Anchor/Ejemplo: '{ex}')\n"
    ...
    resultado = await ejecutar_tarea(prompt_maestro, config_rol, script_origen="brain.extraction_strategies")
...
def supervisar_codigo(script_codigo: str, campos_objetivo: List[str], config: Dict[str, Any]) -> Dict[str, Any]:
    """Auditoria estatica basica (no usa LLM salvo si quisieramos, aqui es python puro)."""
    riesgos = []
    palabras_peligrosas = ["eval(", "exec(", "os.system", "subprocess", "shutil.rmtree", "__import__"]
    for p in palabras_peligrosas:
        if p in script_codigo:
            riesgos.append(f"CRITICO: Uso de funcion peligrosa '{p}' detectada.")

    whitelist_libs = {
        "fitz", "pdfplumber", "pandas", "re", "json", "unicodedata",
        "datetime", "math", "collections", "typing", "io", "openpyxl"
    }

    try:
        imports = re.findall(r'^(?:import|from)\s+([a-zA-Z0-9_]+)', script_codigo, re.MULTILINE)
        ...
        if librerias_desconocidas:
            for lib in librerias_desconocidas:
                riesgos.append(f"ADVERTENCIA: Libreria '{lib}' no esta en la lista blanca.")
    except Exception as e:
        riesgos.append(f"Error al analizar imports: {str(e)}")

    nivel_riesgo = "Alto" if any("CRITICO" in r for r in riesgos) else ("Medio" if riesgos else "Bajo")
    ...
    if aprobado:
        analisis += "Sin funciones peligrosas detectadas.\nTodas las librerias son conocidas."
    else:
...
async def generar_reporte_forense(referencia_ia: Dict, resultado_script: Any, config: Dict[str, Any], system_prompt: str) -> str:
    """
    ...
    """
    config_rol = config.get('supervision', config) if isinstance(config, dict) and 'supervision' in config else config
    ...
    prompt_forense = system_prompt.format(
        ref_json=ref_json,
        script_json=script_json
    )

    resultado = await ejecutar_tarea(prompt_forense, config_rol, script_origen="brain.extraction_strategies")
    return resultado.get("response", "**Error al generar informe.**")

async def regenerar_script_iterativo(script_actual: str, reporte_forense: str, feedback_history: List[Dict], texto_documento: str, config: Dict[str, Any], system_prompt: str, campos_objetivo: List[str] = None) -> str:
    """Regenera script con feedback."""

    if not system_prompt:
        raise ValueError("El prompt de sistema es obligatorio para la Regeneracion de Scripts.")
    ...
    for i, item in enumerate(feedback_history):
        comentario = item.get('comentario', str(item)) if isinstance(item, dict) else str(item)
    ...
    resultado = await ejecutar_tarea(prompt_reparacion, config_rol, script_origen="brain.extraction_strategies")
    if "response" in resultado:
        return _limpiar_y_extraer_codigo_python(resultado["response"])
    else:
...
async def filtrar_datos_irrelevantes(datos_crudos: Dict[str, Any], definicion_usuario: str, config: Dict[str, Any], system_prompt: str) -> Dict[str, Any]:
    """
    ...
    # 5. Ejecutar LLM
    resultado = await ejecutar_tarea(prompt_completo, config_rol, script_origen="brain.extraction_strategies")
    ...
    if "response" in resultado:
        ...
        # Fail-safe
        print(f"[AIBrain] Error en filtro de ruido (Tier 2): {resultado.get('error')}. Devolviendo original.")
        return datos_crudos

async def extraer_dato_desde_snippet(campo: str, snippet_texto: str, formato_esperado: str, config: Dict[str, Any], system_prompt: str) -> Dict[str, Any]:
    """
    ...
    prompt_snippet = system_prompt.format(
        campo=campo,
        snippet_texto=snippet_texto[:4000],
        formato=formato_esperado
    ) + instruction

    resultado = await ejecutar_tarea(prompt_snippet, config_rol, script_origen="brain.strategies.fallback")
...
async def extract_field_from_snippet(field_name: str, snippet_text: str, expected_format: str, role: str, executor) -> Dict[str, Any]:
...
"""
    ...
    try:
        ...
        if fmt == "iban" and not re.search(r'\b[A-Z]{2}\d{2}[A-Z0-9]{10,}\b', val): ok = False
        if fmt == "nif" and not re.search(r'\b[XYZ]?\d{7}[A-Z]\b', val, re.I): ok = False
        if fmt == "date" and not re.search(r'\b\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}\b', val): ok = False
        if fmt in ("money","importe") and not re.search(r'\d{1,3}(\.\d{3})*(,\d{2})', val): ok = False

        data["format_ok"] = bool(ok)
        ph = data.get("pagina")
        data["pagina"] = int(ph) if isinstance(ph,(int,float)) else None
        try: data["confidence"] = float(data.get("confidence", 0.0))
        except: data["confidence"] = 0.0
