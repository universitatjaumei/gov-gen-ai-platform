...
def registrar_log_tokens(proveedor, modelo, tokens_enviados, tokens_recibidos, script_origen="gui.py"):
    ...
    try:
        ...
        try:
            ...
            loop.create_task(log_token_usage(
...
async def ejecutar_tarea(prompt: str, config_rol: Dict[str, Any], script_origen: str ="core.llm") -> Dict[str, Any]:
    """
    ...
    if hasattr(config_rol, "provider"): proveedor = config_rol.provider
    else: proveedor = config_rol.get('proveedor') or config_rol.get('provider')

    if hasattr(config_rol, "model_id"): modelo = config_rol.model_id
    else: modelo = config_rol.get('modelo') or config_rol.get('model_id')
    ...
    for attempt in range(max_retries):
        try:
            if proveedor.lower() == "google":
                ...
                if not api_key: return {"error": "API Key de Google no encontrada en BD"}
                ...
                if final_response and final_response.usage_metadata:
                    ...
                    tokens_recibidos = final_response.usage_metadata.candidates_token_count

                await log_token_usage(script_origen, proveedor, modelo, tokens_enviados, tokens_recibidos)
                return {"response": response_text}

            elif proveedor.lower() == "openrouter":
                api_key = await get_api_key('openrouter')
                if not api_key: return {"error": "API Key de OpenRouter no encontrada en BD"}
            ...
            error_str = str(e)
            print(f"[LLM Gateway] Error with {proveedor} (Attempt {attempt+1}/{max_retries}): {error_str}")

            # Special handling for DNS / Connectivity issues
            if "getaddrinfo" in error_str or "connection" in error_str.lower():
                print(f"[LLM Gateway] Network error detected. Running diagnostic pre-check...")
                import socket
                host_to_check = "generativelanguage.googleapis.com" if proveedor.lower() == "google" else "openrouter.ai"
                try:
                    addr = socket.gethostbyname(host_to_check)
                    print(f"[LLM Gateway] Diagnostic: {host_to_check} resolved to {addr}. DNS is OK now.")
                except Exception as dns_e:
                    print(f"[LLM Gateway] Diagnostic: FAILED to resolve {host_to_check}. DNS/Network issue confirmed: {dns_e}")

            # Check if retryable (Internal Error, 500, 503, Gateway Timeout, or transient DNS issue)
            retry_reason = None
            if "500" in error_str or "503" in error_str or "Internal" in error_str.upper():
            ...
            if retry_reason:
                if attempt < max_retries - 1:
                    ...
                    print(f"[LLM Gateway] {retry_reason}. Retrying in {sleep_time} seconds...")
