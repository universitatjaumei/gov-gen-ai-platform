...
class AIConfig(SQLModel, table=True):
    ...
    """
    role_key: str = Field(primary_key=True, description="Unique role key (e.g., 'extraccion_pdf', 'supervision')")
...
class ExtractionServiceConfig(SQLModel, table=True):
    ...
    """
    service_id: str = Field(primary_key=True, description="Internal ID (e.g., 'sys_phase0_discovery')")
    name: str = Field(description="Human-readable name")
    module: str = Field(default="extraction", description="Module this prompt belongs to")
    target_function: Optional[str] = Field(default=None, description="Function that uses this prompt")
    description: Optional[str] = None
    system_prompt_template: str = Field(sa_column=Column(Text), description="The prompt template")
    expected_schema: Dict[str, Any] = Field(default={}, sa_column=Column(JSON))
    suggested_model: Optional[str] = Field(default=None, description="Recommended model for this prompt")
...
class ModelPricing(SQLModel, table=True):
    ...
    """
    id: str = Field(primary_key=True, description="Model ID (e.g., 'google/gemini-1.5-flash')")
...
class ModelCache(SQLModel, table=True):
    ...
    """
    provider: str = Field(primary_key=True, description="Provider: 'google', 'openrouter', 'ollama'")
...
class AutomationLibrary(SQLModel, table=True):
    ...
    is_workflow: bool = Field(default=False) # Distingue script simple de flujo complejo
...
class License(SQLModel, table=True):
    ...
    max_seats: int = Field(default=1, description="Número máximo de dispositivos únicos permitidos")
...
class LicenseActivation(SQLModel, table=True):
    ...
    machine_id: str = Field(index=True, description="Identificador único del hardware del cliente")
    device_name: Optional[str] = Field(default=None, description="Nombre descriptivo del dispositivo (ej: 'PC de Juan')")
    activated_at: datetime = Field(default_factory=datetime.utcnow, description="Fecha de primera activación")
    last_seen: datetime = Field(default_factory=datetime.utcnow, description="Última conexión del dispositivo")
...
class ScriptEscalation(SQLModel, table=True):
    ...
    escalation_type: str = Field(default="extraction", description="extraction, custom_script")
...
class ServerSecurityPolicy(SQLModel, table=True):
    ...
    partner_id: Optional[str] = Field(default=None, foreign_key="partneraccount.partner_id", index=True)
    client_id: Optional[str] = Field(default=None, foreign_key="clientaccount.client_id", index=True)
...
class SchedulerConfig(SQLModel, table=True):
    ...
    refresh_minute: int = Field(default=0, description="Minute to run daily refresh (0-59)")
    last_run: Optional[datetime] = Field(default=None, description="Timestamp of last successful run")
    updated_at: datetime = Field(default_factory=datetime.utcnow)
...
class SystemPrompt(SQLModel, table=True):
    ...
    # Identification
    name: str = Field(index=True, unique=True, description="Unique ID: 'script_generator', 'flow_orchestrator'")
    version: str = Field(default="1.0", description="Prompt versioning")
    
    # Content
    content: str = Field(sa_column=Column(Text), description="Full text of the system prompt")
    
    # Context type
    context_type: str = Field(
        default="generic",
        description="Type: 'catalog_aware', 'generic', 'copilot'"
    )
    
    # Tier constraints
    tier: Optional[str] = Field(
        default=None,
        description="Required tier: 'FLASH', 'PRO', 'ULTRA' (None = all)"
    )
...
class ClientTelemetryLog(SQLModel, table=True):
    ...
    manifest_id: str = Field(index=True, description="ID único de ejecución generado en cliente")
    ...
    synced_at: datetime = Field(default_factory=datetime.utcnow, description="Hora de recepción en servidor")
