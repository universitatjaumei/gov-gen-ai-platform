...
def admin_dashboard_content():
    ...
    async def load_kpis():
        async with AsyncSession(server_engine) as session:
            ...
            # Active Licenses
            stmt = select(func.count(License.license_id)).where(
    ...
    def render_card(card_data):
        ...
        with (
            ui.card()
            .classes(
                f"w-full p-3 hover:shadow-md transition-all border-l-4 border-{color}-500 flex-row items-center gap-4 cursor-pointer shadow-sm"
            )
            ...
            with ui.column().classes("flex-grow gap-0"):
                ...
                ui.label(desc).classes(
                    "text-[10px] text-gray-400 leading-tight line-clamp-1"
                )

            ui.button(
                icon="arrow_forward", on_click=lambda: ui.navigate.to(route)
    ...
    def kpi_container():
        ...
        with ui.grid().classes(
            "w-full grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3"
        ):
            # KPI 1: Partners
            with ui.card().classes(
                "p-3 flex-row gap-3 items-center bg-white shadow-sm border border-gray-100 hover:shadow-md transition-shadow"
            ):
                with ui.column().classes("p-2 bg-indigo-50 rounded-full"):
                    ui.icon("business", size="1.5em").classes("text-indigo-600")
                with ui.column().classes("gap-0"):
                    ui.label(str(dash_state.total_partners)).classes(
                        "text-base font-bold leading-none text-slate-700"
                    )
                    ui.label(t("admin.dashboard.kpi_partners")).classes(
                        "text-[10px] text-gray-400 uppercase font-bold tracking-wider"
                    )
            ...
            with ui.card().classes(
                ...
                with ui.column().classes("gap-0"):
                    ...
                    ui.label(t("admin.dashboard.kpi_clients")).classes(
                        "text-[10px] text-gray-400 uppercase font-bold tracking-wider"
                    )
            ...
            with ui.card().classes(
                ...
                with ui.column().classes("gap-0"):
                    ...
                    ui.label(t("admin.dashboard.kpi_licenses")).classes(
                        "text-[10px] text-gray-400 uppercase font-bold tracking-wider"
                    )
            ...
            with ui.card().classes(
                ...
                with ui.column().classes("gap-0"):
                    ...
                    ui.label(t("admin.dashboard.kpi_tokens")).classes(
                        "text-[10px] text-gray-400 uppercase font-bold tracking-wider"
                    )

    # --- RENDER MAIN LAYOUT ---

    ui.label(t("admin.dashboard.title")).classes(
    ...
    ui.label(t("admin.dashboard.subtitle")).classes(
