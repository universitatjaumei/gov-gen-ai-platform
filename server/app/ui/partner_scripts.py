...
def partner_scripts_content(ctx: PartnerContext):
    ...
    async def publish_script():
        ...
        ui.notify(f"Script '{state.selected_escalation.script_name}' publicado correctamente", type='positive')
    ...
    async def reject_escalation():
        ...
        async with AsyncSession(server_engine) as session:
            service = PartnerScriptsService(session, ctx.partner_id)
            await service.reject_escalation(state.selected_escalation.id, state.reject_reason)
    ...
    def list_container():
        with ui.row().classes('w-full items-center justify-between mb-4'):
            ...
            ui.button(icon='refresh', on_click=load_escalations).props('flat round color=primary')
        
        if not state.escalations:
            with ui.card().classes('w-full p-6 bg-slate-50 border-dashed border-2 border-slate-200 items-center justify-center'):
                ui.icon('assignment_turned_in', size='lg', color='slate-300')
                ui.label("No hay scripts pendientes de revisión.").classes('text-slate-400 italic mt-2')
            return

        with ui.column().classes('w-full gap-2'):
            for esc in state.escalations:
                with ui.card().classes('w-full p-2 cursor-pointer hover:bg-indigo-50').on('click', lambda e=esc: select_escalation(e)):
                    with ui.row().classes('items-center justify-between'):
                        with ui.column().classes('gap-0'):
                            script_name = esc.script_name or "Sin nombre"
                            ui.label(script_name).classes('font-bold')
                            ui.label(f"Cliente: {esc.client_id}").classes('text-xs text-gray-500')
        ...
        with ui.column().classes('w-full border-t border-slate-200 mt-2 pt-4'):
            with ui.row().classes('w-full items-center justify-between mb-4'):
                with ui.column().classes('gap-1'):
                    script_name = esc.script_name or "Sin nombre"
                    ui.label(f"Revisando: {script_name}").classes('text-xl font-bold text-slate-800')
                    with ui.row().classes('items-center gap-2'):
                        ui.badge(f"Cliente: {esc.client_id}", color='slate-500').props('outline')
                        ui.badge(esc.escalation_type, color='indigo-500').props('outline')
                    
                    if esc.client_notes:
                        with ui.card().classes('bg-amber-50 p-3 border-l-4 border-amber-400 w-full mt-2'):
                            ui.label("Notas del cliente:").classes('text-xs font-bold text-amber-800 uppercase')
                            ui.label(esc.client_notes).classes('text-sm text-amber-900')
                
                with ui.row().classes('gap-3'):
                    ui.button("Rechazar", color="red", on_click=lambda: setattr(state, 'show_reject_dialog', True)).props('flat')
                    ui.button("Publicar corrección", icon='publish', color="primary", on_click=publish_script).classes('px-6')
            ...
            with ui.grid().classes('w-full grid-cols-2 gap-4 h-[500px]'):
                ...
                with ui.column().classes('h-full'):
                    ui.label("Original (solo lectura)").classes('font-bold text-slate-500 text-xs uppercase tracking-wider')
                    code_to_show = esc.original_code or "# No se proporcionó código original"
                    ui.codemirror(code_to_show, language='python').props('readOnly').classes('h-full border border-slate-200 rounded-lg overflow-hidden')

                # Editor (Editable)
                with ui.column().classes('h-full'):
                    ui.label("Corrección (editar)").classes('font-bold text-primary text-xs uppercase tracking-wider')
                    editor = ui.codemirror(state.new_code, language='python', on_change=lambda e: setattr(state, 'new_code', e.value)).classes('h-full border border-primary/30 rounded-lg overflow-hidden')

    # 3. DIALOGS
    with ui.dialog().bind_visibility_from(state, 'show_reject_dialog') as reject_dialog, ui.card().classes('w-[400px] p-6'):
        ui.label("Rechazar solicitud").classes('text-xl font-bold mb-2')
        ui.label("Indica el motivo por el que no se puede procesar esta corrección:").classes('text-sm text-slate-500 mb-4')
        ui.textarea(label="Mensaje para el cliente", placeholder="Ej: No hay suficiente contexto para resolver el error...").bind_value(state, 'reject_reason').classes('w-full mb-6').props('outlined')
        with ui.row().classes('justify-end w-full gap-2'):
            ui.button("Cancelar", on_click=reject_dialog.close).props('flat color=slate-500')
            ui.button("Confirmar rechazo", color="red", on_click=reject_escalation).props('unelevated')
