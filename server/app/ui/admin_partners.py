...
def admin_partners_content():
    t = state.i18n.t
    ui.label(t('admin.partners.title')).classes('text-2xl font-bold mb-6 text-slate-800')
    
    with ui.card().classes('w-full p-4'):
        ui.label(t('admin.partners.title')).classes('text-lg font-bold mb-4')
        ui.label('Integradores tecnológicos del sistema B2B2B').classes('text-sm text-gray-500 mb-4')
        
        # Tabla de Partners
        columns = [
            {'name': 'partner_id', 'label': t('admin.partners.partner_id'), 'field': 'partner_id', 'sortable': True, 'align': 'left'},
            {'name': 'name', 'label': t('admin.partners.partner_name'), 'field': 'name', 'sortable': True, 'align': 'left'},
            {'name': 'email', 'label': t('admin.partners.partner_email'), 'field': 'email', 'align': 'left'},
            {'name': 'credits', 'label': t('admin.partners.credits_balance'), 'field': 'credits_display', 'sortable': True, 'align': 'right'},
            {'name': 'status', 'label': t('admin.partners.partner_status'), 'field': 'status', 'sortable': True, 'align': 'center'},
            {'name': 'actions', 'label': t('admin.common.actions'), 'field': 'actions', 'align': 'center'},
        ]
        
        partner_table = ui.table(columns=columns, rows=[], row_key='partner_id').classes('w-full')
        ...
        with create_dialog, ui.card().classes('w-full max-w-md'):
            ...
            new_partner_name = ui.input(label=t('admin.partners.partner_name'), placeholder='Consultora TIC SL').classes('w-full').props('outlined dense')
            new_partner_email = ui.input(label=t('admin.partners.partner_email'), placeholder='admin@tic.com').classes('w-full').props('outlined dense')
            new_credits = ui.number(label=t('admin.partners.credits_balance'), value=0, format='%d').classes('w-full').props('outlined dense')
            ...
            async def save_new_partner():
                ...
                async with AsyncSession(server_engine) as session:
                    ...
                    try:
                        ...
                        ui.notify(f"{t('admin.partners.partner_created')} (ID: {new_partner.partner_id})", type='positive')
            ...
            with ui.row().classes('w-full justify-end gap-2 mt-4'):
                ui.button(t('admin.common.cancel'), on_click=create_dialog.close).props('flat')
                ui.button(t('admin.common.save'), icon='save', on_click=save_new_partner).props('color=primary')
        
        # --- EDIT DIALOG ---
        with edit_dialog, ui.card().classes('w-full max-w-md'):
            ui.label(t('admin.common.edit')).classes('text-xl font-bold mb-4')
            
            edit_partner_id = ui.input(label=t('admin.partners.partner_id')).classes('w-full').props('outlined dense readonly')
            edit_partner_name = ui.input(label=t('admin.partners.partner_name')).classes('w-full').props('outlined dense')
            edit_partner_email = ui.input(label=t('admin.partners.partner_email')).classes('w-full').props('outlined dense')
            ...
            with ui.row().classes('w-full justify-end gap-2 mt-4'):
                ui.button(t('admin.common.cancel'), on_click=edit_dialog.close).props('flat')
                ui.button(t('admin.common.save'), icon='save', on_click=save_edit_partner).props('color=primary')
        ...
        with credits_dialog, ui.card().classes('w-full max-w-md'):
            ...
            credits_amount_input = ui.number(
                label=t('admin.partners.credits_amount'),
                value=0,
                format='%d'
            ).classes('w-full').props('outlined dense')
                ...
                async with AsyncSession(server_engine) as session:
                    ...
                    if partner:
                        if credits_mode.value == 'add':
                            partner.credits_balance += int(credits_amount_input.value or 0)
                        else:  # set
                            partner.credits_balance = int(credits_amount_input.value or 0)
            ...
            with ui.row().classes('w-full justify-end gap-2 mt-4'):
                ...
                ui.button(t('save'), icon='save', on_click=save_credits).props('color=primary')
        ...
        with consumption_dialog, ui.card().classes('w-full max-w-3xl'):
            ...
            def render_consumption_content():
                ...
                with ui.row().classes('w-full items-center gap-4 mb-4'):
                    ...
                    ui.button(icon='refresh', on_click=reload_consumption).props('flat dense')
                ...
                with ui.row().classes('w-full gap-4 mb-4'):
                    with ui.card().classes('flex-1 p-4 bg-blue-50'):
                        ...
                        ui.label(f"{data.get('total_tokens', 0):,}").classes('text-2xl font-bold text-blue-800')
                ...
                with ui.row().classes('w-full gap-4'):
                    ...
                    with ui.card().classes('flex-1 p-0'):
                        ...
                        client_columns = [
                            ...
                            {'name': 'cost_usd', 'label': 'USD', 'field': 'cost_usd', 'sortable': True, 'align': 'right'},
                        ]
                        ui.table(
        ...
        async def delete_partner(row):
            async with AsyncSession(server_engine) as session:
                ...
                if partner:
                    ...
                    stmt = select(ClientAccount).where(ClientAccount.partner_id == partner.partner_id)
                    result = await session.exec(stmt)
                    clients = result.all()
                    
                    if clients:
                        ui.notify('No se puede eliminar: tiene clientes asociados', type='warning')
                        return
        ...
        partner_table.add_slot('body-cell-status', r'''
            <q-td :props="props">
        ...
        partner_table.add_slot('body-cell-actions', r'''
            <q-td :props="props">
        ...
        async def refresh_partners():
            async with AsyncSession(server_engine) as session:
                statement = select(PartnerAccount).order_by(desc(PartnerAccount.created_at))
        ...
        with ui.row().classes('w-full justify-between mt-4'):
            ui.button(t('admin.partners.add_partner'), icon='add', on_click=create_dialog.open).props('color=primary')
            ui.button(t('admin.common.update'), icon='refresh', on_click=refresh_partners).props('flat dense')
