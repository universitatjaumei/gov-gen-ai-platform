...
class PartnerContext:
    ...
    async def from_session(
        ...
        if not partner_id:
            ui.notify('DEV MODE: Usando sesión simulada (dev-partner-0000)', type='warning')
...
def get_sidebar_items() -> List[Dict[str, Any]]:
    ...
    t = state.i18n.t
    return [
        {"label": t('partner.menu_dashboard'), "icon": "dashboard", "route": "/partner/dashboard"},
        {"label": t('partner.menu_clients'), "icon": "people", "route": "/partner/clients"},
        {"label": t('partner.menu_licenses'), "icon": "verified_user", "route": "/partner/licenses"},
        {"label": t('partner.menu_security'), "icon": "security", "route": "/partner/security"},
        {"label": "Biblioteca", "icon": "library_books", "route": "/partner/library"},
        {"label": t('partner.menu_scripts'), "icon": "code", "route": "/partner/scripts"},
        {"label": t('partner.menu_billing'), "icon": "receipt_long", "route": "/partner/billing"},
    ]
...
def partner_layout(content_fn: Callable, ctx: PartnerContext):
    ...
    with ui.header().classes('bg-slate-900 text-white shadow-md items-center'):
        with ui.row().classes('w-full items-center justify-between px-4'):
            ...
            with ui.row().classes('items-center gap-4'):
                ...
                ui.label(ctx.partner_name).classes('text-sm font-semibold truncate max-w-[150px]')
                ui.button(icon='logout', on_click=lambda: ui.navigate.to('/partner/logout')).props('flat round color=white')
    ...
    with ui.row().classes('w-full h-[calc(100vh-64px)]'):
        ...
        with ui.column().classes('w-64 h-full bg-slate-50 flex flex-col p-0 gap-0 border-r border-gray-200'):
            ui.label("MENU PRINCIPAL").classes('text-gray-500 text-xs font-bold px-4 py-2 uppercase tracking-wider mt-2')
        ...
        with ui.column().classes('flex-1 h-full p-6 bg-white overflow-y-auto'):
            ...
            with ui.row().classes('mb-4 gap-2 text-sm text-gray-500'):
                bc = generate_breadcrumb(app.storage.browser.get('current_path', '/partner'))
