...
from server.app.database.models import PartnerAccount, ClientAccount, ServerSecurityPolicy as SecurityPolicy
...
def admin_security_content():
    ...
    with system_policy_dialog, ui.card().classes('w-full max-w-2xl'):
        ...
        with ui.column().classes('w-full gap-4'):
            # Domains
            ui.label(t('admin.security.allowed_domains')).classes('font-semibold text-gray-700')
            sys_allow_all = ui.checkbox(t('admin.security.allow_all_domains'), value=True)
            sys_domains_area = ui.textarea(
                placeholder='ejemplo.com\n*.midominio.es',
            ).props('outlined rows=3').classes('w-full')
            sys_domains_area.bind_visibility_from(sys_allow_all, 'value', lambda v: not v)
            ...
            with ui.row().classes('w-full gap-4'):
                with ui.column().classes('flex-1'):
                    ui.label(t('admin.security.allowed_libraries')).classes('font-semibold text-gray-700')
                    sys_allowed_libs = ui.textarea(
                        value='pandas\njson\nre\nmath\ndatetime\nopenpyxl\nxlrd\nnumpy',
                        placeholder='Una librería por línea'
                    ).props('outlined rows=6').classes('w-full')

                with ui.column().classes('flex-1'):
                    ui.label(t('admin.security.forbidden_libraries')).classes('font-semibold text-gray-700')
                    sys_forbidden_libs = ui.textarea(
                        value='os\nsys\nsubprocess\nrequests\nsocket\nshutil\nctypes',
                        placeholder='Una librería por línea'
                    ).props('outlined rows=6').classes('w-full')
            ...
            # Resource Limits
            ui.label(t('admin.security.max_execution_time')).classes('font-semibold text-gray-700')
            sys_max_time = ui.slider(min=30, max=1800, step=30, value=300).classes('w-full')
            sys_time_label = ui.label('300 segundos').classes('text-sm text-gray-500')
            sys_max_time.on('update:model-value', lambda e: sys_time_label.set_text(f'{e.args} segundos'))

            ui.label(t('admin.security.max_memory')).classes('font-semibold text-gray-700')
            sys_max_mem = ui.slider(min=128, max=4096, step=128, value=512).classes('w-full')
            sys_mem_label = ui.label('512 MB').classes('text-sm text-gray-500')
            sys_max_mem.on('update:model-value', lambda e: sys_mem_label.set_text(f'{e.args} MB'))

            ui.separator()

            # Screenshot Policy
            ui.label("Política de Capturas de Pantalla (RPA)").classes('font-semibold text-gray-700')
            sys_screenshot_policy = ui.radio(
                options={
            ...
            ui.label("Dominios de Confianza (Screenshot)").classes('text-sm text-gray-600 q-mt-sm')
            sys_trusted_domains = ui.textarea(
                placeholder='boe.es\n*.gob.es'
            ).props('outlined rows=2').classes('w-full')
            # Only show if TRUSTED is selected
            sys_trusted_domains.bind_visibility_from(sys_screenshot_policy, 'value', lambda v: v == 'TRUSTED')

        async def save_system_policy():
            async with AsyncSession(server_engine) as session:
                service = SecurityPolicyService(session)

                domains = ['*'] if sys_allow_all.value else [
                    d.strip() for d in sys_domains_area.value.split('\n') if d.strip()
                ]
                allowed = [lib.strip() for lib in sys_allowed_libs.value.split('\n') if lib.strip()]
                forbidden = [lib.strip() for lib in sys_forbidden_libs.value.split('\n') if lib.strip()]
                ...
                await service.save_system_policy(
                    ...
                    trusted_screenshot_domains=[d.strip() for d in sys_trusted_domains.value.split('\n') if d.strip()]
                )
        ...
        with ui.row().classes('w-full justify-end gap-2 mt-4'):
            ui.button(t('admin.common.cancel'), on_click=system_policy_dialog.close).props('flat')
            ui.button(t('admin.common.save'), icon='save', on_click=save_system_policy).props('color=primary')
    ...
    with partner_policy_dialog, ui.card().classes('w-full max-w-2xl'):
        ...
        with ui.column().classes('w-full gap-4'):
            # Domains
            ui.label(t('admin.security.allowed_domains')).classes('font-semibold text-gray-700')
            partner_allow_all = ui.checkbox(t('admin.security.allow_all_domains'), value=True)
            partner_domains_area = ui.textarea(
                placeholder='ejemplo.com\n*.midominio.es',
            ).props('outlined rows=3').classes('w-full')
            partner_domains_area.bind_visibility_from(partner_allow_all, 'value', lambda v: not v)
            ...
            with ui.row().classes('w-full gap-4'):
                with ui.column().classes('flex-1'):
                    ui.label(t('admin.security.allowed_libraries')).classes('font-semibold text-gray-700')
                    partner_allowed_libs = ui.textarea(
                        value='pandas\njson\nre\nmath\ndatetime\nopenpyxl\nxlrd\nnumpy',
                        placeholder='Una librería por línea'
                    ).props('outlined rows=6').classes('w-full')
                ...
                with ui.column().classes('flex-1'):
                    ui.label(t('admin.security.forbidden_libraries')).classes('font-semibold text-gray-700')
                    partner_forbidden_libs = ui.textarea(
                        value='os\nsys\nsubprocess\nrequests\nsocket\nshutil\nctypes',
                        placeholder='Una librería por línea'
                    ).props('outlined rows=6').classes('w-full')
            ...
            # Resource Limits
            ui.label(t('admin.security.max_execution_time')).classes('font-semibold text-gray-700')
            partner_max_time = ui.slider(min=30, max=1800, step=30, value=300).classes('w-full')
            partner_time_label = ui.label('300 segundos').classes('text-sm text-gray-500')
            partner_max_time.on('update:model-value', lambda e: partner_time_label.set_text(f'{e.args} segundos'))

            ui.label(t('admin.security.max_memory')).classes('font-semibold text-gray-700')
            partner_max_mem = ui.slider(min=128, max=4096, step=128, value=512).classes('w-full')
            partner_mem_label = ui.label('512 MB').classes('text-sm text-gray-500')
            partner_max_mem.on('update:model-value', lambda e: partner_mem_label.set_text(f'{e.args} MB'))

            ui.separator()

            # Screenshot Policy
            ui.label("Política de Capturas de Pantalla (RPA)").classes('font-semibold text-gray-700')
            partner_screenshot_policy = ui.radio(
                options={
                    'BLOCK': 'Bloquear Todo (Alta Seguridad)',
                    'REVIEW': 'Revisión Obligatoria (Recomendado)',
                    'TRUSTED': 'Dominios de Confianza'
            ...
            ui.label("Dominios de Confianza (Screenshot)").classes('text-sm text-gray-600 q-mt-sm')
            partner_trusted_domains = ui.textarea(
                placeholder='boe.es\n*.gob.es'
            ).props('outlined rows=2').classes('w-full')
            # Only show if TRUSTED is selected
            partner_trusted_domains.bind_visibility_from(partner_screenshot_policy, 'value', lambda v: v == 'TRUSTED')

        async def save_partner_policy():
            async with AsyncSession(server_engine) as session:
                service = SecurityPolicyService(session)

                domains = ['*'] if partner_allow_all.value else [
                    d.strip() for d in partner_domains_area.value.split('\n') if d.strip()
                ]
                allowed = [lib.strip() for lib in partner_allowed_libs.value.split('\n') if lib.strip()]
                forbidden = [lib.strip() for lib in partner_forbidden_libs.value.split('\n') if lib.strip()]
                ...
                await service.save_partner_policy(
                    ...
                    trusted_screenshot_domains=[d.strip() for d in partner_trusted_domains.value.split('\n') if d.strip()]
                )
        ...
        with ui.row().classes('w-full justify-end gap-2 mt-4'):
            ui.button(t('admin.common.cancel'), on_click=partner_policy_dialog.close).props('flat')
            ui.button(t('admin.common.save'), icon='save', on_click=save_partner_policy).props('color=primary')
    ...
    with client_policy_dialog, ui.card().classes('w-full max-w-2xl'):
        ...
        with ui.column().classes('w-full gap-4'):
            # Domains
            ui.label(t('admin.security.allowed_domains')).classes('font-semibold text-gray-700')
            client_allow_all = ui.checkbox(t('admin.security.allow_all_domains'), value=True)
            client_domains_area = ui.textarea(
                placeholder='ejemplo.com\n*.midominio.es',
            ).props('outlined rows=3').classes('w-full')
            client_domains_area.bind_visibility_from(client_allow_all, 'value', lambda v: not v)
            ...
            with ui.row().classes('w-full gap-4'):
                with ui.column().classes('flex-1'):
                    ui.label(t('admin.security.allowed_libraries')).classes('font-semibold text-gray-700')
                    client_allowed_libs = ui.textarea(
                        value='pandas\njson\nre\nmath\ndatetime\nopenpyxl\nxlrd\nnumpy',
                        placeholder='Una librería por línea'
                    ).props('outlined rows=6').classes('w-full')
                ...
                with ui.column().classes('flex-1'):
                    ui.label(t('admin.security.forbidden_libraries')).classes('font-semibold text-gray-700')
                    client_forbidden_libs = ui.textarea(
                        value='os\nsys\nsubprocess\nrequests\nsocket\nshutil\nctypes',
                        placeholder='Una librería por línea'
                    ).props('outlined rows=6').classes('w-full')
            ...
            # Resource Limits
            ui.label(t('admin.security.max_execution_time')).classes('font-semibold text-gray-700')
            client_max_time = ui.slider(min=30, max=1800, step=30, value=300).classes('w-full')
            client_time_label = ui.label('300 segundos').classes('text-sm text-gray-500')
            client_max_time.on('update:model-value', lambda e: client_time_label.set_text(f'{e.args} segundos'))

            ui.label(t('admin.security.max_memory')).classes('font-semibold text-gray-700')
            client_max_mem = ui.slider(min=128, max=4096, step=128, value=512).classes('w-full')
            client_mem_label = ui.label('512 MB').classes('text-sm text-gray-500')
            client_max_mem.on('update:model-value', lambda e: client_mem_label.set_text(f'{e.args} MB'))

            ui.separator()

            # Screenshot Policy
            ui.label("Política de Capturas de Pantalla (RPA)").classes('font-semibold text-gray-700')
            client_screenshot_policy = ui.radio(
                options={
                    'BLOCK': 'Bloquear Todo (Alta Seguridad)',
                    'REVIEW': 'Revisión Obligatoria (Recomendado)',
                    'TRUSTED': 'Dominios de Confianza'
            ...
            ui.label("Dominios de Confianza (Screenshot)").classes('text-sm text-gray-600 q-mt-sm')
            client_trusted_domains = ui.textarea(
                placeholder='boe.es\n*.gob.es'
            ).props('outlined rows=2').classes('w-full')
            # Only show if TRUSTED is selected
            client_trusted_domains.bind_visibility_from(client_screenshot_policy, 'value', lambda v: v == 'TRUSTED')

        async def save_client_policy():
            async with AsyncSession(server_engine) as session:
                service = SecurityPolicyService(session)

                domains = ['*'] if client_allow_all.value else [
                    d.strip() for d in client_domains_area.value.split('\n') if d.strip()
                ]
                allowed = [lib.strip() for lib in client_allowed_libs.value.split('\n') if lib.strip()]
                forbidden = [lib.strip() for lib in client_forbidden_libs.value.split('\n') if lib.strip()]
                ...
                await service.save_client_policy(
                    ...
                    trusted_screenshot_domains=[d.strip() for d in client_trusted_domains.value.split('\n') if d.strip()]
                )
        ...
        with ui.row().classes('w-full justify-end gap-2 mt-4'):
            ui.button(t('admin.common.cancel'), on_click=client_policy_dialog.close).props('flat')
            ui.button(t('admin.common.save'), icon='save', on_click=save_client_policy).props('color=primary')

    # === MAIN CONTENT ===
    ui.label(t('admin.security.title')).classes('text-2xl font-bold mb-6 text-slate-800')
    ...
    with ui.card().classes('w-full p-4 mb-4'):
        with ui.row().classes('w-full justify-between items-center mb-4'):
            ...
            ui.button(t('admin.common.edit'), icon='edit', on_click=lambda: open_system_policy_dialog()).props('flat color=primary')
            ...
            with system_policy_display:
                with ui.grid(columns=4).classes('w-full gap-4'):
                    with ui.card().classes('p-3 bg-blue-50'):
                        ui.label(t('admin.security.allowed_domains')).classes('text-xs text-gray-500 uppercase')
                        domains = data.get('allowed_domains', [])
                        if domains == ['*']:
                            ui.label('Todos los dominios').classes('font-bold text-blue-700')
                        else:
                            ui.label(f'{len(domains)} dominios').classes('font-bold text-blue-700')

                    with ui.card().classes('p-3 bg-green-50'):
                        ui.label(t('admin.security.allowed_libraries')).classes('text-xs text-gray-500 uppercase')
                        ui.label(f'{len(data.get("allowed_libraries", []))} librerías').classes('font-bold text-green-700')

                    with ui.card().classes('p-3 bg-red-50'):
                        ui.label(t('admin.security.forbidden_libraries')).classes('text-xs text-gray-500 uppercase')
                        ui.label(f'{len(data.get("forbidden_libraries", []))} librerías').classes('font-bold text-red-700')

                    with ui.card().classes('p-3 bg-orange-50'):
                        ui.label('Límites').classes('text-xs text-gray-500 uppercase')
                        ui.label(f'{data.get("max_execution_time", 300)}s / {data.get("max_memory_mb", 512)}MB').classes('font-bold text-orange-700')
    ...
    with ui.card().classes('w-full p-4 mb-4'):
        with ui.row().classes('w-full justify-between items-center mb-4'):
            with ui.row().classes('items-center gap-2'):
                ...
                ui.label(t('admin.security.partner_policies')).classes('text-lg font-bold')

        partners_columns = [
            {'name': 'partner_id', 'label': 'Partner ID', 'field': 'partner_id', 'align': 'left'},
            {'name': 'name', 'label': 'Nombre', 'field': 'name', 'align': 'left'},
            {'name': 'policy_status', 'label': t('admin.security.policy_scope'), 'field': 'policy_status', 'align': 'center'},
            {'name': 'actions', 'label': t('admin.common.actions'), 'field': 'actions', 'align': 'center'},
        ]

        partners_table = ui.table(columns=partners_columns, rows=[], row_key='partner_id').classes('w-full')

        partners_table.add_slot('body-cell-policy_status', r'''
            <q-td :props="props">
        ...
        partners_table.add_slot('body-cell-actions', r'''
            <q-td :props="props">
    ...
    with ui.card().classes('w-full p-4'):
        with ui.row().classes('w-full justify-between items-center mb-4'):
            with ui.row().classes('items-center gap-2'):
                ...
                ui.label(t('admin.security.client_policies')).classes('text-lg font-bold')
        ...
        clients_columns = [
            ...
            {'name': 'name', 'label': 'Nombre', 'field': 'client_name', 'align': 'left'},
            {'name': 'partner_name', 'label': 'Partner', 'field': 'partner_name', 'align': 'left'},
            {'name': 'policy_level', 'label': t('admin.security.effective_policy'), 'field': 'policy_level', 'align': 'center'},
            {'name': 'actions', 'label': t('admin.common.actions'), 'field': 'actions', 'align': 'center'},
        ]

        clients_table = ui.table(columns=clients_columns, rows=[], row_key='client_id').classes('w-full')

        clients_table.add_slot('body-cell-policy_level', r'''
            <q-td :props="props">
        ...
        clients_table.add_slot('body-cell-actions', r'''
            <q-td :props="props">
                <q-btn size="sm" flat dense icon="edit" color="primary"
    ...
    async def refresh_partners_table():
        async with AsyncSession(server_engine) as session:
            # Get all partners
            result = await session.exec(select(PartnerAccount).where(PartnerAccount.is_active == True))
            partners = result.all()
    ...
    async def refresh_clients_table():
        async with AsyncSession(server_engine) as session:
            ...
            if selected_partner['id']:
                ...
                stmt = select(ClientAccount).where(ClientAccount.is_active == True)
    ...
    async def open_system_policy_dialog():
        async with AsyncSession(server_engine) as session:
            ...
            sys_allowed_libs.value = '\n'.join(service._parse_json(policy.allowed_libraries))
            sys_forbidden_libs.value = '\n'.join(service._parse_json(policy.forbidden_libraries))
            ...
            trusted = service._parse_json(getattr(policy, 'trusted_screenshot_domains', '[]'))
    ...
    async def open_partner_policy_dialog(row):
        ...
        async with AsyncSession(server_engine) as session:
            ...
            if policy:
                ...
                partner_domains_area.value = '' if domains == ['*'] else '\n'.join(domains)
                ...
                # Populate Screenshot Policy
                partner_screenshot_policy.value = getattr(policy, 'screenshot_policy', 'REVIEW')
                trusted = service._parse_json(getattr(policy, 'trusted_screenshot_domains', '[]'))
                ...
                partner_allow_all.value = domains == ['*']
                partner_domains_area.value = '' if domains == ['*'] else '\n'.join(domains)
                partner_allowed_libs.value = '\n'.join(service._parse_json(system_policy.allowed_libraries))
                partner_forbidden_libs.value = '\n'.join(service._parse_json(system_policy.forbidden_libraries))
                ...
                # Populate defaults 
                partner_screenshot_policy.value = getattr(system_policy, 'screenshot_policy', 'REVIEW')
                trusted = service._parse_json(getattr(system_policy, 'trusted_screenshot_domains', '[]'))
                partner_trusted_domains.value = '\n'.join(trusted)
    ...
    async def open_client_policy_dialog(row):
        ...
        async with AsyncSession(server_engine) as session:
            ...
            if policy:
                ...
                client_domains_area.value = '' if domains == ['*'] else '\n'.join(domains)
                ...
                # Populate Screenshot Policy
                client_screenshot_policy.value = getattr(policy, 'screenshot_policy', 'REVIEW')
                trusted = service._parse_json(getattr(policy, 'trusted_screenshot_domains', '[]'))
                ...
                client_allow_all.value = domains == ['*']
                client_domains_area.value = '' if domains == ['*'] else '\n'.join(domains)
                client_allowed_libs.value = '\n'.join(effective.get('allowed_libraries', []))
                client_forbidden_libs.value = '\n'.join(effective.get('forbidden_libraries', []))
                client_max_time.value = effective.get('max_execution_time', 300)
                client_time_label.text = f'{effective.get("max_execution_time", 300)} segundos'
                ...
                client_screenshot_policy.value = effective.get('screenshot_policy', 'REVIEW')
