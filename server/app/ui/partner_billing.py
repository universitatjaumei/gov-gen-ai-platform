...
def partner_billing_content(ctx: PartnerContext):
    ...
    async def load_data():
        async with AsyncSession(server_engine) as session:
            ...
            page_state.summary = await service.get_monthly_summary(page_state.selected_year, page_state.selected_month)
    ...
    async def export_csv():
        async with AsyncSession(server_engine) as session:
            service = PartnerBillingService(session, ctx.partner_id)
            csv_data = await service.export_to_csv(page_state.selected_year, page_state.selected_month)
            
        ui.download(csv_data.encode(), f"billing_{page_state.selected_year}_{page_state.selected_month}.csv")
    ...
    def render_dashboard():
        ...
        with ui.row().classes('items-center gap-4 mb-6'):
            ...
            ui.button(t('partner.billing.refresh'), icon="refresh", on_click=load_data).props('flat')
            ui.button(t('partner.billing.export_csv'), icon="download", on_click=export_csv).props('color=green')

        # KPI Cards
        with ui.row().classes('w-full gap-4 mb-6'):
            with ui.card().classes('flex-1 p-4 bg-blue-50'):
                ui.label(t('partner.billing.total_tokens')).classes('text-gray-600')
                ui.label(f"{page_state.summary.get('total_tokens', 0):,}").classes('text-3xl font-bold text-blue-800')
            
            with ui.card().classes('flex-1 p-4 bg-green-50'):
                ui.label(t('partner.billing.estimated_cost')).classes('text-gray-600')
                ui.label(f"${page_state.summary.get('total_cost_usd', 0):.2f}").classes('text-3xl font-bold text-green-800')

        # Alerts
        if page_state.alerts:
            with ui.expansion(t('partner.billing.alerts_title'), icon="warning", value=True).classes('w-full bg-red-50 mb-6 text-red-800'):
        ...
        with ui.grid(columns='1fr 1fr').classes('w-full gap-6'):
            ...
            with ui.card().classes('p-0'):
                ui.label(t('partner.billing.by_client')).classes('p-4 font-bold border-b')
                
                columns = [
                    {'name': 'client_name', 'label': t('partner.billing.col_client'), 'field': 'client_name', 'align': 'left'},
                    {'name': 'tokens', 'label': t('partner.billing.col_tokens'), 'field': 'tokens', 'sortable': True},
                    {'name': 'cost_usd', 'label': t('partner.billing.col_cost'), 'field': 'cost_usd', 'sortable': True},
                ]
                ui.table(columns=columns, rows=page_state.summary.get('by_client', []), row_key='client_id').classes('w-full no-shadow')

            # By Model
            with ui.card().classes('p-0'):
                ui.label(t('partner.billing.by_model')).classes('p-4 font-bold border-b')
                
                columns_model = [
                    {'name': 'model_id', 'label': t('partner.billing.col_model'), 'field': 'model_id', 'align': 'left'},
                    {'name': 'tokens', 'label': t('partner.billing.col_tokens'), 'field': 'tokens', 'sortable': True},
                    {'name': 'cost_usd', 'label': t('partner.billing.col_cost'), 'field': 'cost_usd', 'sortable': True},
                ]
                ui.table(columns=columns_model, rows=page_state.summary.get('by_model', []), row_key='model_id').classes('w-full no-shadow')
