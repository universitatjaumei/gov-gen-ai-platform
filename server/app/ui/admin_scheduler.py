...
def admin_scheduler_content():
    ...
    ui.label(t('admin.scheduler.title')).classes('text-2xl font-bold mb-6 text-slate-800')
    ...
    with ui.card().classes('w-full p-4'):
        ...
        with ui.column().classes('w-full gap-4'):
            ...
            with ui.row().classes('items-center gap-4'):
                ui.label(t('admin.scheduler.enabled_label')).classes('text-sm font-medium')
    ...
    async def refresh_status():
        ...
        status_card.clear()
        with status_card:
            ui.label(t('admin.scheduler.status_title')).classes('text-lg font-bold mb-4')

            with ui.grid(columns=2).classes('w-full gap-4'):
                # Status indicator
                with ui.column().classes('items-center p-4 bg-slate-50 rounded'):
                    status_color = 'text-green-600' if status['running'] else 'text-gray-400'
                    status_icon = 'check_circle' if status['running'] else 'cancel'
                    ui.icon(status_icon).classes(f'{status_color} text-4xl')
                    ui.label(
                        t('admin.scheduler.status_running') if status['running']
                ...
                with ui.column().classes('items-center p-4 bg-slate-50 rounded'):
                    ...
                    if status['next_run']:
                        ...
                        ui.label(next_run.strftime('%H:%M')).classes('text-blue-600 font-bold text-xl')
                        ui.label(t('admin.scheduler.next_run')).classes('text-gray-500 text-sm')
                    else:
                        ui.label('--:--').classes('text-gray-400 font-bold text-xl')
                        ui.label(t('admin.scheduler.not_scheduled')).classes('text-gray-400 text-sm')
