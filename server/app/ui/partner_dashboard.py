...
def partner_dashboard_content(ctx: PartnerContext):
    ...
    async def load_kpis():
        async with AsyncSession(server_engine) as session:
            ...
            stmt = select(func.count(License.license_id)).join(
    ...
    def render_card(card_data):
        ...
        with ui.card().classes(f'w-full h-full p-0 hover:shadow-lg transition-all border-t-4 border-{color}-500 flex flex-col cursor-pointer').on('click', lambda: ui.navigate.to(route)):
            # Header
            ...
            with ui.row().classes('w-full p-3 border-t border-gray-100 justify-end bg-slate-50'):
                ui.button(t('partner.dashboard.open'), icon='arrow_forward', on_click=lambda: ui.navigate.to(route)) \
    ...
    def kpi_container():
        ...
        with ui.grid().classes('w-full grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'):
            # KPI 1: Clientes
            with ui.card().classes('p-4 bg-white'):
                with ui.row().classes('items-center gap-3'):
                    ui.icon('people', size='2em').classes('text-indigo-500')
                    with ui.column().classes('gap-0'):
                        ui.label(str(dash_state.total_clients)).classes('text-3xl font-bold')
                        ui.label(t('partner.dashboard.kpi_clients')).classes('text-xs text-gray-500 uppercase')
            ...
            with ui.card().classes('p-4 bg-white'):
                with ui.row().classes('items-center gap-3'):
                    ...
                    with ui.column().classes('gap-0'):
                        ui.label(str(dash_state.active_licenses)).classes('text-3xl font-bold')
                        ui.label(t('partner.dashboard.kpi_licenses')).classes('text-xs text-gray-500 uppercase')
            ...
            with ui.card().classes('p-4 bg-white'):
                with ui.row().classes('items-center gap-3'):
                    ...
                    with ui.column().classes('gap-0'):
                        ui.label(f"{dash_state.total_tokens_month:,}").classes('text-3xl font-bold')
                        ui.label(t('partner.dashboard.kpi_tokens')).classes('text-xs text-gray-500 uppercase')
            ...
            with ui.card().classes('p-4 bg-white'):
                with ui.row().classes('items-center gap-3'):
                    ...
                    with ui.column().classes('gap-0'):
                        ui.label(str(dash_state.pending_scripts)).classes('text-3xl font-bold')
                        ui.label(t('partner.dashboard.kpi_scripts')).classes('text-xs text-gray-500 uppercase')

    # --- RENDER MAIN LAYOUT ---
    
    ui.label(t('partner.dashboard.title')).classes('text-2xl font-bold mb-6 text-slate-800')

    kpi_container()
    
    ui.separator().classes('my-8')
    
    ui.label(t('partner.dashboard.subtitle')).classes('text-xl font-bold mb-4 text-slate-700')
