...
from server.app.database.models import TokenLog, ModelPricing, PartnerAccount, ClientAccount, AutomationLibrary, ClientTelemetryLog
...
def admin_analytics_content(ctx: AdminContext):
    ...
    async def get_global_stats():
        async with AsyncSession(server_engine) as session:
            # 1. Counts
            partners = (await session.exec(select(func.count(PartnerAccount.partner_id)))).one()
            clients = (await session.exec(select(func.count(ClientAccount.client_id)))).one()
            automations = (await session.exec(select(func.count(AutomationLibrary.id)))).one()
            
            # 2. Recent Token Logs
            recent_logs = (await session.exec(select(TokenLog).order_by(TokenLog.timestamp.desc()).limit(10))).all()
    ...
    async def get_telemetry_logs(client_filter: str = None, status_filter: str = 'ALL', days_filter: int = 7):
        async with AsyncSession(server_engine) as session:
            query = select(ClientTelemetryLog).order_by(ClientTelemetryLog.timestamp_client.desc()).limit(100)
            
            if client_filter:
                query = query.where(col(ClientTelemetryLog.client_id).contains(client_filter))
    ...
    filter_client = ui.input('Buscar Cliente ID').classes('w-48').props('clearable dense')
    filter_status = ui.select(['ALL', 'success', 'error'], value='ALL', label='Estado').classes('w-32').props('dense options-dense')
    filter_days = ui.number(label='Días', value=7, min=1, max=30).classes('w-20').props('dense')

    async def render_dashboard():
        container.clear()
        
        with container:
            ui.label("Panel de Observabilidad Check").classes('text-2xl font-bold mb-4 text-primary')
    ...
    async def render_global_tab():
                ...
                with ui.card().classes(f'w-64 p-4 border-l-4 border-{color}-500'):
                    with ui.row().classes('items-center justify-between w-full'):
                            with ui.column().classes('gap-0'):
                                ui.label(title).classes('text-gray-500 text-sm font-bold uppercase')
        ...
        with ui.card().classes('w-full p-4 mb-6'):
            ui.label("Actividad Reciente de Tokens (TokenLog)").classes('text-lg font-bold mb-4')
    ...
    async def render_telemetry_tab():
        with ui.card().classes('w-full p-4'):
            ...
            with ui.row().classes('w-full items-center gap-4 mb-4'):
                ...
                ui.button(icon='refresh', on_click=refresh_click).props('flat round color=primary')
            ...
            async def update_telemetry_table():
                ...
                with table_container:
                     if not rows:
                         ui.label("No se encontraron registros.").classes('text-gray-500 italic')
                         return

                     ui.table(
                        columns=[
                            {'name': 'time', 'label': 'Fecha/Hora', 'field': 'time', 'sortable': True, 'align': 'left'},
                            {'name': 'client', 'label': 'Cliente', 'field': 'client', 'sortable': True, 'align': 'left'},
                            {'name': 'service', 'label': 'Servicio', 'field': 'service', 'sortable': True, 'align': 'left'},
                            {'name': 'status', 'label': 'Estado', 'field': 'status', 'sortable': True, 'align': 'center'},
                            {'name': 'duration', 'label': 'Tiempo', 'field': 'duration', 'sortable': True, 'align': 'right'},
