...
def get_status_label(license_obj: License) -> str:
    ...
    if license_obj.valid_until and datetime.utcnow().date() > license_obj.valid_until.date(): return "EXPIRED"
...
def admin_clients_content():
    ...
    with ui.card().classes('w-full p-4'):
        ...
        ui.label('Vista jerárquica Partner → Cliente → Licencia').classes('text-sm text-gray-500 mb-4')
        ...
        with ui.row().classes('w-full gap-4 mb-4 items-end'):
            ...
            async def apply_filters():
                filter_state['partner_id'] = partner_filter.value if partner_filter.value else None
        ...
        # Tabla de Clientes
        columns = [
            {'name': 'client_id', 'label': t('admin.clients.client_id'), 'field': 'client_id', 'sortable': True, 'align': 'left'},
            {'name': 'name', 'label': t('admin.clients.client_name'), 'field': 'name', 'sortable': True, 'align': 'left'},
            {'name': 'partner', 'label': t('admin.clients.associated_partner'), 'field': 'partner_name', 'sortable': True, 'align': 'left'},
            {'name': 'status', 'label': t('admin.partners.partner_status'), 'field': 'license_status', 'sortable': True, 'align': 'center'},
            {'name': 'quota', 'label': t('admin.clients.quota_tokens'), 'field': 'quota_display', 'align': 'right'},
            {'name': 'actions', 'label': t('common.actions'), 'field': 'actions', 'align': 'center'},
        ]
        
        client_table = ui.table(columns=columns, rows=[], row_key='client_id').classes('w-full')
        ...
        with create_client_dialog, ui.card().classes('w-full max-w-2xl'):
            ...
            with ui.stepper().props('vertical').classes('w-full') as stepper:
                ...
                with ui.step('Datos Básicos'):
                    new_client_id = ui.input(label=t('admin.clients.client_id'), placeholder='client_001').classes('w-full').props('outlined dense')
                    new_client_name = ui.input(label=t('admin.clients.client_name'), placeholder='Ayuntamiento de Valencia').classes('w-full').props('outlined dense')
                    ...
                    async def load_partners():
                        async with AsyncSession(server_engine) as session:
                            stmt = select(PartnerAccount).where(PartnerAccount.is_active == True)
                            result = await session.exec(stmt)
                            partners = result.all()
                            
                            partner_options.clear()
                            for p in partners:
                                partner_options[p.partner_id] = f"{p.name} ({p.partner_id})"
                            partner_select.update()
                    
                    ui.timer(0.1, load_partners, once=True)
                    
                    with ui.stepper_navigation():
                        ui.button('Siguiente', on_click=stepper.next).props('color=primary')
                        ui.button(t('common.cancel'), on_click=create_client_dialog.close).props('flat')
                
                # STEP 2: License Key
                with ui.step('License Key'):
                    ui.markdown('### Generación de License Key').classes('mb-2')
                    ui.label('La key se mostrará UNA SOLA VEZ. Guárdala de forma segura.').classes('text-sm text-orange-600 mb-4')
                    
                    generated_key_display = ui.input(
                    ...
                    with ui.row().classes('w-full gap-2'):
                        ui.button(t('admin.clients.generate_key'), icon='vpn_key', on_click=generate_license_key).props('color=primary')
                        
                        def copy_key():
                            if generated_key_display.value:
                                ui.run_javascript(f'navigator.clipboard.writeText("{generated_key_display.value}")')
                                ui.notify(t('admin.clients.key_copied'), type='positive')
                        
                        ui.button(t('admin.clients.copy_to_clipboard'), icon='content_copy', on_click=copy_key).props('flat')
                    
                    # Warning Box
                    with ui.card().classes('w-full bg-yellow-50 border border-yellow-300 p-4 mt-4'):
                        ui.label(t('admin.clients.key_warning')).classes('text-yellow-800 font-bold')
                        ui.label('Esta clave se guardará hasheada (SHA256) en la base de datos y no será recuperable.').classes('text-sm text-yellow-700')
                ...
                with ui.step('Configuración Licencia'):
                    new_license_id = ui.input(
                        label=t('admin.clients.license_id'),
                        placeholder='lic_001'
                    ).classes('w-full').props('outlined dense')
                    ...
                    new_valid_until = ui.input(
                        label=t('admin.clients.valid_until'),
                        value=default_date
                    ...
                    with ui.stepper_navigation():
                        ...
                        async def save_client_and_license():
                            ...
                            if not new_client_id.value or not new_client_name.value:
                                ui.notify('ID y Nombre son obligatorios', type='negative')
                                return
                            
                            if not partner_select.value:
                                ui.notify('Debes seleccionar un Partner', type='negative')
                                return
                            
                            if not generated_key_value['key']:
                                ui.notify('Debes generar una License Key', type='negative')
                                return
                            
                            if not new_license_id.value:
                                ui.notify('ID de Licencia es obligatorio', type='negative')
                                return
                            ...
                            async with AsyncSession(server_engine) as session:
                                # Check duplicates
                                existing_client = await session.get(ClientAccount, new_client_id.value)
                                if existing_client:
                                    ui.notify('Ya existe un cliente con ese ID', type='negative')
                                    return
                                
                                existing_license = await session.get(License, new_license_id.value)
                                if existing_license:
                                    ui.notify('Ya existe una licencia con ese ID', type='negative')
                                    return
                                ...
                                license = License(
                                    ...
                                    consumed_tokens=0,
                                    valid_until=datetime.strptime(new_valid_until.value, '%Y-%m-%d'),
                            ...
                            ui.notify(t('admin.clients.client_created'), type='positive')
                            create_client_dialog.close()
                            await refresh_clients()
                            
                            # Reset form
                            stepper.value = stepper.props['model-value'] = 'Datos Básicos'
                            new_client_id.value = ''
                        ...
                        ui.button('Crear Cliente', icon='save', on_click=save_client_and_license).props('color=primary')
        
        # --- TABLE ACTIONS ---
        client_table.add_slot('body-cell-status', r'''
            <q-td :props="props">
        ...
        client_table.add_slot('body-cell-actions', r'''
            <q-td :props="props">
            ...
            def render_detail_content():
                ...
                with ui.grid(columns=2).classes('w-full gap-4 mb-4'):
                    with ui.column():
                        ...
                        ui.label(f"{client['name']} ({client['client_id']})").classes('font-bold')
                    with ui.column():
                ...
                usage_pct = (license['consumed'] / license['quota'] * 100) if license['quota'] > 0 else 0
                ui.label(f"Uso de Cuota: {license['consumed']:,} / {license['quota']:,} tokens ({usage_pct:.1f}%)").classes('text-sm mt-2')
                ...
                if consumption:
                    with ui.row().classes('w-full gap-8 mb-4'):
                        with ui.card().classes('p-4 bg-blue-50'):
                            ...
                            ui.label(f"{consumption.get('total_tokens', 0):,}").classes('text-2xl font-bold text-blue-800')
                    ...
                    ui.label('Sin datos de consumo disponibles').classes('text-gray-500')
            ...
            async with AsyncSession(server_engine) as session:
                ...
                if license:
                    detail_state['license'] = {
                        ...
                        'valid_until': license.valid_until.strftime('%Y-%m-%d') if license.valid_until else 'N/A'
                    }
        ...
        async def refresh_clients():
            async with AsyncSession(server_engine) as session:
                statement = select(ClientAccount, License, PartnerAccount)\
                ...
                if filter_state['partner_id']:
                    statement = statement.where(ClientAccount.partner_id == filter_state['partner_id'])
                ...
                for client, license, partner in data:
                    ...
                    if filter_state['search_query']:
                        ...
                        if search_lower not in client.name.lower() and search_lower not in client.client_id.lower():
                            continue
        ...
        with ui.row().classes('w-full justify-between mt-4'):
            ui.button(t('admin.clients.add_client'), icon='add', on_click=create_client_dialog.open).props('color=primary')
            ui.button(t('common.update'), icon='refresh', on_click=refresh_clients).props('flat dense')
