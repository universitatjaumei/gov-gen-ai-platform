...
def partner_licenses_content(ctx: PartnerContext):
    ...
    async def adjust_quota():
        if not state.selected_license:
            return

        async with AsyncSession(server_engine) as session:
            service = PartnerLicenseService(session, ctx.partner_id)
            await service.adjust_quota(
                state.selected_license.license_id, int(state.add_tokens)
            )
    ...
    async def extend_validity():
        ...
        async with AsyncSession(server_engine) as session:
            service = PartnerLicenseService(session, ctx.partner_id)
            await service.extend_validity(
                state.selected_license.license_id, int(state.add_days)
            )
            ...
            async with AsyncSession(server_engine) as session:
                ...
                if lic.status == LicenseStatus.ACTIVE.value:
                    await service.suspend_license(
                        lic.license_id, reason="Manual suspension"
                    )
    ...
    with ui.column().classes("w-full gap-6"):
            ...
            with ui.row().classes("w-full gap-4 mb-4"):
                if state.alerts_expiring:
                    with ui.card().classes(
                        "bg-red-50 border-l-4 border-red-500 p-4 flex-1"
                    ):
                        with ui.row().classes("items-center gap-2"):
                            ui.icon("warning", size="sm").classes("text-red-600")
                            ui.label("Expira Pronto").classes("font-bold text-red-800")
                        ui.label(
                            f"{len(state.alerts_expiring)} licencias expiran en < 15 dias"
                        ).classes("text-sm text-red-700")

                if state.alerts_quota:
                    with ui.card().classes(
                        "bg-orange-50 border-l-4 border-orange-500 p-4 flex-1"
                    ):
                        with ui.row().classes("items-center gap-2"):
                            ui.icon("battery_alert", size="sm").classes(
                                "text-orange-600"
                            )
                            ui.label("Quota Baja").classes("font-bold text-orange-800")
                        ui.label(
                            f"{len(state.alerts_quota)} licencias con < 20% quota"
        ...
        def license_table():
            ...
            if not state.licenses:
                ui.label("No hay licencias activas.").classes(
                    "w-full text-center italic text-gray-500"
                )
                return
            ...
            with ui.card().classes("w-full p-0 gap-0"):
                ...
                with ui.row().classes(
                    ...
                    ui.label("Acciones").classes(
                        "flex-1 text-right min-w-[150px]"
                    )  # Ensure minimum width for icons

                # Rows
                for lic in state.licenses:
                    with ui.row().classes(
                        "w-full p-3 border-b hover:bg-slate-50 items-center"
                    ):
                        ...
                        with ui.column().classes(
                            "w-1/5 gap-1"
                        ):  # Reduced from 1/4 (matches header)
                            pct = 0
                            ...
                            ui.label(
                                f"{lic.consumed_tokens:,} / {lic.quota_tokens:,}"
                        ...
                        ui.label(
                            f"{lic.valid_until.strftime('%Y-%m-%d')} ({days_left} dias)"
                        ).classes(f"w-1/6 text-sm {valid_color}")

                        # Status
                        is_active = lic.status.upper() == "ACTIVE"
                        status_color = "green" if is_active else "red"
                        ui.badge(lic.status.upper(), color=status_color).classes(
                            "w-1/6"
                        )
                        ...
                        with ui.row().classes("flex-1 justify-end gap-1"):
                            ui.button(
                                icon="bar_chart",
                                on_click=lambda license_item=lic: load_consumption(license_item),
                            ).props("flat dense round color=teal").tooltip(
                                "Ver Consumo"
                            )
                            ui.button(
                                icon="add_circle",
                                on_click=lambda license_item=lic: open_quota_dialog(license_item),
                            ).props("flat dense round color=blue").tooltip(
                                "Aumentar Quota"
                            )
                            ui.button(
                                icon="event_repeat",
                                on_click=lambda license_item=lic: open_validity_dialog(license_item),
                            ).props("flat dense round color=green").tooltip(
                                "Extender Validez"
                            )

                            pause_icon = (
                                "play_arrow" if lic.status == "SUSPENDED" else "pause"
                            )
                            ui.button(
                                icon=pause_icon, on_click=lambda license_item=lic: toggle_status(license_item)
                            ).props("flat dense round color=orange").tooltip(
                                "Suspender/Reactivar"
                            )

                            ui.button(
                                icon="history", on_click=lambda license_item=lic: load_audit(license_item)
                            ).props("flat dense round color=grey").tooltip(
                                "Ver Auditoría"
                            )
        ...
        def quota_dialog():
            with ui.dialog() as dialog, ui.card():
                ...
                if state.selected_license:
                    ui.label(f"Licencia: {state.selected_license.license_id}").classes(
                        "text-sm text-gray-500 mb-2"
                    )

                ui.number("Tokens a añadir", value=50000, step=1000).bind_value(
                    state, "add_tokens"
        ...
        def validity_dialog():
            with ui.dialog() as dialog, ui.card():
                ...
                ui.number("Días a añadir", value=30, step=1).bind_value(
                    state, "add_days"
                ).classes("w-full").props("outlined")

                with ui.row().classes("w-full justify-end mt-4"):
                    ui.button("Cancelar", on_click=dialog.close).props("flat")
                    ui.button("Aplicar", on_click=extend_validity).props(
                        "color=primary"
                    )
                ...
                if not state.audit_logs:
                    ...
                    with ui.column().classes("w-full gap-2 max-h-96 overflow-y-auto"):
                        for log in state.audit_logs:
                            with ui.row().classes(
                                "w-full p-2 border rounded items-start gap-3 bg-gray-50"
                            ):
                                ui.label(
                                    log["timestamp"][:16].replace("T", " ")
                                ).classes("text-xs font-mono text-gray-500 w-24")
                                with ui.column().classes("flex-1 gap-0"):
                                    ...
                                    details_str = json.dumps(
                                        log["details"], ensure_ascii=False
                                    )
                                    ui.label(details_str).classes(
                                        "text-xs text-gray-600 break-all"
                                    )
        ...
        def consumption_dialog():
            with ui.dialog() as dialog, ui.card().classes("w-full max-w-3xl"):
                ...
                if not data:
                    ...
                    # Info básica
                    with ui.row().classes("w-full gap-4 mb-4"):
                        ui.label(f"Cliente: {data.get('client_name', 'N/A')}").classes(
                            "font-bold"
                        )
                        ui.label(f"Licencia: {data.get('license_id', 'N/A')}").classes(
                            "text-gray-500"
                        )
                    ...
                    with ui.row().classes("w-full gap-4 mb-4"):
                        with ui.card().classes("flex-1 p-4 bg-blue-50"):
                            ui.label("Tokens Consumidos").classes(
                                "text-sm text-gray-600"
                            )
                            ui.label(f"{data.get('consumed_tokens', 0):,}").classes(
                        ...
                        with ui.card().classes("flex-1 p-4 bg-purple-50"):
                            ui.label("Tokens Restantes").classes(
                                "text-sm text-gray-600"
                            )
                            ui.label(f"{data.get('remaining_tokens', 0):,}").classes(
                    ...
                    ui.linear_progress(value=usage_pct / 100).props(
                        f"color={color}"
                    ...
                    if ops:
                        ...
                        with ui.column().classes("w-full max-h-64 overflow-y-auto"):
                            for op in ops:
                                with ui.row().classes(
                                    "w-full p-2 border-b text-sm items-center"
                                ):
                                    ui.label(op.get("timestamp", "")).classes(
                                        "w-32 text-gray-500 font-mono text-xs"
                                    )
                                    ui.label(op.get("model_id", "N/A")).classes(
                                        "flex-1"
                                    )
                                    ui.label(f"{op.get('tokens_used', 0):,}").classes(
                                        "w-24 text-right"
                                    )
                                    ui.label(f"${op.get('cost_usd', 0):.6f}").classes(
                                        "w-24 text-right text-green-600"
                                    )
                    else:
                        ui.label("No hay operaciones registradas").classes(
