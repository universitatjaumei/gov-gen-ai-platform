...
def admin_library_content(ctx: AdminContext):
    ...
    async def delete_item():
        ...
        async with AsyncSession(server_engine) as session:
            service = LibraryService(session)
            # Superadmin deletion
            success = await service.delete_automation(state.selected_item.id, "system", is_superadmin=True)
    ...
    @ui.refreshable
    def list_container():
        ui.label("Biblioteca Global (System Templates)").classes('text-2xl font-bold mb-4')
        ui.button("Nuevo Template", icon="add", on_click=create_new).classes('mb-4 bg-slate-800 text-white')
        ...
        with ui.column().classes('w-full gap-2'):
            for item in state.items:
                with ui.card().classes('w-full p-2 cursor-pointer hover:bg-slate-50 border-l-4 border-slate-500').on('click', lambda i=item: select_item(i)):
    ...
    def detail_container():
        ...
        with ui.column().classes('w-full gap-4'):
            ...
            ui.label("Definición (Python/JSON)").classes('text-sm font-bold text-gray-600')
            ui.codemirror(language='python').bind_value(state, 'code').classes('h-96 border shadow-inner')
            
            with ui.row().classes('w-full justify-end gap-2'):
                if state.selected_item:
                    ui.button("Eliminar (Forzar)", color="red", on_click=delete_item).props('outline')
