...
try:
    ...
    print("[AgentService] browser-use or langchain not installed. Agent functionality unavailable.")
    Agent = None
...
class BrowserAgentWrapper:
    ...
    async def _get_supervision_llm(self):
        ...
        async with AsyncSession(server_engine) as session:
            ...
            if config:
                ...
                print(f"[AgentService] Loaded from DB: provider={provider}, model={model_id}")
            else:
                # Default fallback if not configured - log warning
                provider = "google"
                model_id = "gemini-2.0-flash-exp"
                print(f"[AgentService] WARNING: Role 'supervision' NOT FOUND in DB. Using hardcoded fallback (google).")

        print(f"[AgentService] Usando modelo de supervision: {provider}/{model_id}")

        if provider == "google":
            api_key = await get_api_key('google')
            if not api_key: raise ValueError("Google API Key no encontrada")
            ...
            api_key = await get_api_key('openrouter') # Ojo: api_key_service usa 'openrouter' como key name
            if not api_key: raise ValueError("OpenRouter API Key no encontrada")
    ...
    async def run_agent_task(self, context: BrowserContext, task_instruction: str) -> AgentResult:
        """
