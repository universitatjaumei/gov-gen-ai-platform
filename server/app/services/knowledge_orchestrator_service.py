...
class KnowledgeOrchestratorService:
    ...
    async def search_remote_inventory(
        self, query: str, client_id: str, partner_id: str, max_results: int = 15
    ) -> List[Dict[str, Any]]:
        """
        ...
        async with AsyncSession(server_engine) as session:
            ...
            stmt = select(AutomationLibrary).where(
                or_(
                    ...
                    AutomationLibrary.is_system_template,
                )
            )
            ...
            if query and query.lower() != "all":
                q = f"%{query.lower()}%"
                stmt = stmt.where(
                    or_(AutomationLibrary.name.ilike(q), AutomationLibrary.id.ilike(q))
                )
            ...
            for auto in automations:
                ...
                ui_contract = (
                    auto.metadata_json.get("ui_contract", {})
                    if auto.metadata_json
                    else {}
                )
                description = (
                    auto.metadata_json.get("description", "")
                    if auto.metadata_json
                    else ""
                )
    ...
    def format_inventory_block(
        self, local_inventory: List[Dict], remote_inventory: List[Dict]
    ) -> str:
        """
    ...
    async def get_system_prompt(self, name: str) -> Optional[str]:
        ...
        async with AsyncSession(server_engine) as session:
            stmt = select(SystemPrompt).where(
                SystemPrompt.name == name, SystemPrompt.is_active
            )
            result = await session.execute(stmt)
            prompt = result.scalars().first()
            return prompt.content if prompt else None

    async def build_hybrid_system_prompt(
        self,
        user_query: str,
        local_inventory: List[Dict],
        client_id: str,
        partner_id: str,
        prompt_name: str = "flow_orchestrator",
    ) -> str:
        """
        ...
        remote_items = await self.search_remote_inventory(
            user_query, client_id, partner_id
        )
