...
from server.app.modules.automation.cortex import (
...
class AIBrainService:
    ...
    async def _resolve_server_config(
        self, service_id: str = None, role_key: str = None
        ...
        async with AsyncSession(server_engine) as session:
            ...
            if service_id:
                stmt = select(ExtractionServiceConfig).where(
                    ExtractionServiceConfig.service_id == service_id
                )
            ...
            if role_config:
                ...
                final_config = {"provider": role_config.provider, "model_id": model_id}
                print(
                    f"[AIBrain] Role '{target_role}' loaded from DB: provider={role_config.provider}, model={model_id}"
                )
            else:
                # Fallback hardcoded for safety during bootstrap
                print(
                    f"[AIBrain] WARNING: Role '{target_role}' NOT FOUND in DB. Using hardcoded fallback (google)."
                )
                final_config = {
                    "provider": "google",
    ...
    async def get_system_prompts(self) -> List[Dict]:
        ...
        async with AsyncSession(server_engine) as session:
            ...
            return [
                ...
                for c in configs
                if str(c.service_id).startswith("sys_")
                or str(c.name).startswith("System:")
            ]

    async def get_service_config(
        self, service_id: str
    ) -> Optional[ExtractionServiceConfig]:
        """
    ...
    async def analyze_recording(
        self, recording_logs: List[Dict], context: Dict[str, Any]
    ) -> List[Dict]:
        """
        ...
        config, system_prompt = await self._resolve_server_config(
            service_id="sys_rpa_analysis", role_key="logico_navegacion"
        )

        if not system_prompt:
            # Fallback logic might be needed if init wasn't run, but we should enforce DB consistecy.
            print(
                "[AIBrainService] Warning: sys_rpa_analysis prompt not found. RPA might fail."
            )
    ...
    async def refine_playbook(
        ...
        config, system_prompt = await self._resolve_server_config(
            service_id="sys_rpa_refinement", role_key="supervision"
        )
    ...
    async def get_element_coordinates(
        self, image_bytes: bytes, element_description: str, viewport: Dict[str, int]
        ...
        # 1. Config Lookup
        config, system_prompt = await self._resolve_server_config(
            service_id="sys_rpa_vision", role_key="logico_navegacion"
    ...
    async def analyze_document_structure(
        ...
        config_fast, sys_fast = await self._resolve_server_config(
            service_id=service_id, role_key="phase0_discovery_fast"
        )

        # Call Strategy (Fast)
        resp = await analyze_document_structure(
            texto_a, texto_b, "es", config_fast, sys_fast
        )

        # 2. Check & Fallback
        if not resp.get("format_ok"):
            print(
                "[AIBrain] Discovery Fast failed (Format Invalid). Escalating to Smartest."
            )

            # Resolve Smart
            config_smart, sys_smart = await self._resolve_server_config(
                service_id=service_id, role_key="phase0_discovery_smart"
            )

            # Call Strategy (Smart)
            resp = await analyze_document_structure(
                texto_a, texto_b, "es", config_smart, sys_smart
            )
    ...
    async def extract_data(
        ...
        config, system_prompt = await self._resolve_server_config(
            service_id="sys_phase1_extraction", role_key=role
        )

        if not system_prompt:
            raise RuntimeError(
                "No se encontro el prompt de sistema sys_phase1_extraction"
            )
    ...
    async def refine_extraction_data(
        ...
        config, system_prompt = await self._resolve_server_config(
            service_id=service_id, role_key="logico_navegacion"
        )
    ...
    async def generate_extraction_script(
        ...
        values_example: Optional[Dict] = None,
        info_discovery: Optional[Dict] = None,
        feedback: str = None,
        ...
        """
        print(
            f"[AIBrainService] Secure Request: Generate Script ({len(docs_text_list)} docs)"
        )

        # 0. Pre-flight Billing Validation
        if license_key:
            from server.app.modules.automation.billing_engine import (
                BillingEngine,
                PartnerCreditError,
                LicenseError,
            )
            ...
            except (PartnerCreditError, LicenseError):
        ...
        config, system_prompt = await self._resolve_server_config(
            service_id="sys_phase3_factory_gen", role_key="logico_navegacion"
        )
        ...
        return await generar_script_determinista(
            ...
            config,
            system_prompt,
            self._sanitize(values_example),
            self._sanitize(info_discovery),
            feedback,
            tabla_auditoria,
            field_definitions=self._sanitize(field_definitions),
        )

    async def clean_data_noise(
        self,
        raw_data: Dict[str, Any],
        user_definition: str,
        service_id: str = "sys_utility_noise_filter",
    ) -> Dict[str, Any]:
        """
        ...
        """
        print(
            f"[AIBrainService] Secure Request: clean_data_noise (Tier 2, Service: {service_id})"
        )
        ...
        return await filtrar_datos_irrelevantes(
            self._sanitize(raw_data), user_definition, config_wrapper, system_prompt
        )

    async def audit_script(
        self, script_codigo: str, campos_objetivo: List[str]
    ) -> Dict[str, Any]:
    ...
    async def refine_script_with_error_logs(
        ...
        print(
            f"[AIBrainService] Secure Request: Refine Script (Targets: {len(target_fields)})"
        )

        # Uses tier 3 (Supervision) + Refinement Prompt
        config, system_prompt = await self._resolve_server_config(
            service_id="sys_phase3_refinement", role_key="supervision"
        )
        ...
        return await regenerar_script_iterativo(
            script_actual,
            reporte_forense,
            self._sanitize(feedback_history),
            texto_documento,
            config,
            system_prompt,
    ...
    async def generate_forensic_audit(
        self, referencia_ia: Dict, resultado_script: Any
    ) -> str:
        """Generacion de reporte forense (Audit)."""
        print(
            f"[AIBrainService] Secure Request: Forensic Audit (Result type: {type(resultado_script)})"
        )

        config, system_prompt = await self._resolve_server_config(
            service_id="sys_phase3_audit_forensic", role_key="supervision"
        )
        ...
        return await generar_reporte_forense(
            self._sanitize(referencia_ia),
            self._sanitize(resultado_script),
            config,
            system_prompt,
        )
    ...
    async def generate_script_anchor_based(
        self,
        doc1_texts: Dict[str, str],
        doc2_texts: Dict[str, str],
        ...
        if not system_prompt:
            ...
            config, system_prompt = await self._resolve_server_config(
                service_id="sys_pdf_extraction", role_key="logico_navegacion"
            )
        ...
        docs_list = [
            {
                "filename": "Documento_Referencia_1.pdf",
                "fitz": doc1_texts.get("fitz", ""),
                "plumber": doc1_texts.get("plumber", ""),
            },
            {
                "filename": "Documento_Validacion_2.pdf",
                "fitz": doc2_texts.get("fitz", ""),
                "plumber": doc2_texts.get("plumber", ""),
            },
        ]
    ...
    async def analyze_snippet(
        self, field_name: str, snippet_text: str, expected_format: str = ""
    ) -> dict:
        """
        ...
        # 1. Tier 1
        config, system_prompt = await self._resolve_server_config(
            service_id="sys_fallback_snippet", role_key="extraccion_pdf"
        )
        if not system_prompt:
            config, system_prompt = await self._resolve_server_config(
                service_id="sys_pdf_extraction", role_key="extraccion_pdf"
            )

        if system_prompt:
            result = await extraer_dato_desde_snippet(
                field_name, snippet_text, expected_format, config, system_prompt
            )
            ...
            if val and str(val).strip() and is_ok:
                return {
                    ...
                    ),  # Default high if it thinks it's ok
                    "source": "llm",
                    "format_ok": True,
                }
            else:
                print(
                    f"[AIBrainService] Tier 1 failed/invalid for '{field_name}'. Escalating."
                )

        # 2. Tier 2
        config_t2, system_prompt_t2 = await self._resolve_server_config(
            service_id="sys_fallback_snippet", role_key="logico_navegacion"
        )
        if not system_prompt_t2:
            print("[AIBrainService] Tier 2 Prompt missing.")
            return {}

        result_t2 = await extraer_dato_desde_snippet(
            field_name, snippet_text, expected_format, config_t2, system_prompt_t2
        )
    ...
    async def extract_field_generic_guided(
        self, field_name: str, snippet_text: str, expected_format: str = "text"
    ) -> Dict[str, Any]:
        """
    ...
    async def _get_license_id_from_key(self, license_key: str) -> Optional[str]:
        ...
        async with AsyncSession(server_engine) as session:
            result = await session.execute(
                ...
                .join(ClientAccount)
                .where(ClientAccount.license_key == key_hash)
            )
    ...
    async def generate_text(
        ...
        await self._validate_license(license_key)
    ...
    async def call_llm(
        ...
        config, system_prompt = await self._resolve_server_config(
            service_id=service_id, role_key=role_key
        )
    ...
    async def _call_llm(
        self, prompt: str, model: str = None, temperature: float = 0.7
    ) -> str:
        """
    ...
    async def _update_token_consumption(self, license_id: str, tokens: int):
        ...
        await billing_engine.record_consumption(
            license_id=license_id, tokens_used=tokens, operation="text_generation"
        )
    ...
    async def verify_device_access(
        self, license_key: str, machine_id: str, device_name: str = None
    ) -> dict:
        """
